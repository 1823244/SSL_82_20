...
Функция ЕстьПраваНаОбновлениеИнформационнойБазы() 
	...
	Если НЕ ОбщегоНазначенияПовтИсп.РазделениеВключено() 
		...
		Возврат ПравоДоступа("МонопольныйРежим", Метаданные) И Пользователи.ЭтоПолноправныйПользователь(, Истина);
	Иначе
		Возврат Истина;
	КонецЕсли;
	...
	ИначеЕсли НеобходимоОбновлениеИнформационнойБазы() И НЕ ЕстьПраваНаОбновлениеИнформационнойБазы() Тогда
		...
		Если ЗначениеЗаполнено(ПользователиИнформационнойБазы.ТекущийПользователь().Имя) Тогда
			Сообщение = НСтр("ru = 'Для завершения обновления конфигурации требуются административные права.
				|Необходимо добавить в список пользователей информационной базы учетную запись администратора с двумя ролями АдминистраторСистемы и ПолныеПрава,
...
Функция ВыполнитьОбновлениеИнформационнойБазы(Знач Принудительно = Ложь) Экспорт
	...
	Если НЕ Принудительно И НЕ НеобходимоВыполнитьОбновление(ВерсияМетаданных, ВерсияДанных) Тогда
...
Функция ПервыйЗапуск() Экспорт
	...
	Иначе
		Возврат ПервыйВходВОбластьДанных();
	КонецЕсли;
...
Процедура СохранитьИнформациюОбОбновленииДляПользователей(Знач ВыполненныеОбработчики)
	...
	Для каждого ПользовательИБ Из ПользователиИнформационнойБазы.ПолучитьПользователей() Цикл
		...
		Иначе
			ДеревоОбработчиков = ВыполненныеРанееОбработчики;
