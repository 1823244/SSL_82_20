...
//
//  Примечание:
//  Перед вызовом процедуры желательно актуализировать кэш МРО. Для этого следует выполнить вызов
//  ОбменДаннымиСервер.ОбновитьПовторноИспользуемыеЗначенияМРОПриНеобходимости();
//
Процедура ПриОтправкеДанных(ЭлементДанных, ОтправкаЭлемента, УзелИнформационнойБазы, Знач ИмяПланаОбмена, Знач СозданиеНачальногоОбраза) Экспорт
	...
	Если Не ОбменДаннымиПовтИсп.РегистрироватьИзмененияПоПравиламРегистрацииОбъекта(ИмяПланаОбмена, ОбъектМетаданных.ПолноеИмя()) Тогда
		Возврат; // фильтрацию не выполняем
	КонецЕсли;
	...
	Если    ИмяБазовогоТипа = ОбщегоНазначения.ИмяТипаСправочники()
		...
		Если НомерВМассиве = Неопределено Тогда
			...
			ОтправкаЭлемента = ?(СозданиеНачальногоОбраза, ОтправкаЭлементаДанных.Игнорировать, ОтправкаЭлементаДанных.Удалить);
...
Процедура ПроверитьВозможностьИзмененияНастроекОбменаДанными(Источник, Отказ) Экспорт
	...
	Если  Не Источник.ДополнительныеСвойства.Свойство("ПолучениеСообщенияОбмена")
		...
		И ОбменДаннымиСервер.ИзмененияЗарегистрированы(Источник.Ссылка)
		Тогда
		
		ПодробноеПредставлениеОшибки = НСтр("ru = 'Настройка обмена данными не может быть изменена.
												|Существуют зарегистрированные изменения данных для обмена.
												|Выполните обмен данными два раза и повторите попытку.'"
		);
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ПодробноеПредставлениеОшибки,,,, Отказ);
		
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Обмен данными'"), УровеньЖурналаРегистрации.Ошибка,, Источник.Ссылка, ПодробноеПредставлениеОшибки);
...
Процедура ПриОтправкеДанныхПодчиненному(Источник, ЭлементДанных, ОтправкаЭлемента, СозданиеНачальногоОбраза) Экспорт
	...
	Если ОбменДаннымиПовтИсп.ЭтоУзелОбменаДаннымиБСП(Источник.Ссылка) Тогда
		
		Если ТипЗнч(ЭлементДанных) = Тип("КонстантаМенеджерЗначения.ИспользоватьОбменДаннымиВЛокальномРежиме") Тогда
			
			ЭлементДанных.Значение = Истина;
			
		ИначеЕсли ТипЗнч(ЭлементДанных) = Тип("КонстантаМенеджерЗначения.ИспользоватьОбменДаннымиВМоделиСервиса") Тогда
			
			ЭлементДанных.Значение = Ложь;
...
Процедура ВыполнитьПравилоРегистрацииОбъектаДляНабораЗаписей(МассивУзловРезультат,
	...
	Если Замещение Тогда
		...
		ОпределитьМассивПолучателейПоНаборуЗаписей(МассивУзловРезультат, СтарыйНаборЗаписей, ПРО, ОбъектМетаданных, ИмяПланаОбмена, Истина, Выгрузка);
...
Функция ОпределитьМассивУзловПоЗначениямСвойств(ЗначенияСвойств, ПРО, Знач ИмяПланаОбмена, Объект, Выгрузка)
...
Процедура ЗаписьЖурналаРегистрацииПРО(Комментарий, ОбъектМетаданных = Неопределено) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Обмен данными.Правила регистрации объектов'"), УровеньЖурналаРегистрации.Ошибка, ОбъектМетаданных, , Комментарий);
