...
//
// Возврвщаемое значение:
//  Строка - полное имя файла с выгрузкой.
//
Функция ВыгрузитьТекущуюОбластьВАрхив(Знач БезДеленияПоФайлам = Ложь, Знач СохранятьПаролиПользователей = Ложь) Экспорт
	...
	ЗаписатьИнформациюОВыгрузкеВXML(КаталогВыгрузки, БезДеленияПоФайлам);
	
	Если БезДеленияПоФайлам Тогда
		ИмяФайлаВыгрузки = КаталогВыгрузки + "data_misc.xml";
		Запись = Новый ЗаписьXML;
		Запись.ОткрытьФайл(ИмяФайлаВыгрузки);
		Запись.ЗаписатьОбъявлениеXML();
		ЗаписатьДанныеОбластиВXML(Запись, Истина);
		Запись.Закрыть();
	Иначе
		ЗаписатьДанныеВXMLСУказаниемТипов(КаталогВыгрузки, ПолучитьМассивТиповПоставляемыхСправочников());
	КонецЕсли;
	
	ЗаписатьПользователейВXML(КаталогВыгрузки, СохранятьПаролиПользователей);
	
	ВыгрузкаЗагрузкаДанныхПереопределяемый.ПередЗавершениемВыгрузки(КаталогВыгрузки);
...
//
Процедура ЗагрузитьТекущуюОбластьИзАрхива(Знач ИмяАрхива, Знач БезДеленияПоФайлам = Ложь, Знач ЗагружатьПользователей = Ложь, Знач РежимОтладки = Ложь) Экспорт
	...
	Если БезДеленияПоФайлам Тогда
		ЗагрузитьДанныеЧерезXDTOСериализатор(КаталогВыгрузки, , РежимОтладки);
	Иначе
		СтандартныеПодсистемыПереопределяемый.СоздатьУзелОбластиДанных(ОбщегоНазначения.ЗначениеРазделителяСеанса());
		
		ЗагрузитьДанныеСПодменойСсылок(КаталогВыгрузки, РежимОтладки);
	КонецЕсли;
...
// Записывает данные области данных в переданный объект ЗаписьXML.
//
// Параметры:
//  Запись - ЗаписьXML, ЗаписьFastInfoSet - инициализированный объект для записи XML.
//  ВыгружатьОбщиеПоставляемыеДанные - Булево - выгружать общие данные, являющиеся
//   общими поставляемые данными.
//
Процедура ЗаписатьДанныеОбластиВXML(Знач Запись, Знач ВыгружатьОбщиеПоставляемыеДанные = Ложь) Экспорт
	
	НевыгружаемыеТипы = Новый Массив;
	СтандартныеПодсистемыПереопределяемый.ПолучитьНевыгружаемыеИзИнформационнойБазыТипы(НевыгружаемыеТипы);
	
	ИменаНевыгружаемыхТипов = Новый Соответствие;
	Для каждого Тип Из НевыгружаемыеТипы Цикл
		ИменаНевыгружаемыхТипов.Вставить(Метаданные.НайтиПоТипу(Тип), Истина);
	КонецЦикла;
	ИменаНевыгружаемыхТипов = Новый ФиксированноеСоответствие(ИменаНевыгружаемыхТипов);
	
	ИменаВыгружаемыхОбщихДанных = Новый Соответствие;
	Если ВыгружатьОбщиеПоставляемыеДанные Тогда
		...
		Для каждого СтрокаСоответствия Из ОбщегоНазначения.ПолучитьТаблицуСоответствияРазделенныхДанныхОбщим() Цикл
			...
			ПолноеИмяОбщихДанных = Метаданные.НайтиПоТипу(СтрокаСоответствия.ТипОбщихДанных);
			Если ИменаНевыгружаемыхТипов.Получить(ПолноеИмяОбщихДанных) = Неопределено Тогда
				ИменаВыгружаемыхОбщихДанных.Вставить(ПолноеИмяОбщихДанных, Истина);
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	ИменаВыгружаемыхОбщихДанных = Новый ФиксированноеСоответствие(ИменаВыгружаемыхОбщихДанных);
	...
	Для каждого МетаданныеКонстанты Из Метаданные.Константы Цикл
		ПолноеИмяОбъектаМД = МетаданныеКонстанты.ПолноеИмя();
		
		Если ИменаНевыгружаемыхТипов.Получить(ПолноеИмяОбъектаМД) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		Если НЕ ОбщегоНазначения.ЭтоРазделенныйОбъектМетаданных(МетаданныеКонстанты) Тогда
			Если ИменаВыгружаемыхОбщихДанных.Получить(ПолноеИмяОбъектаМД) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		...
		Попытка
			ВыгрузкаЗагрузкаДанныхПереопределяемый.ПередВыгрузкойДанных(МенеджерЗначения);
			ЗаписатьXML(Запись, МенеджерЗначения);
		Исключение
			ВызватьИсключениеСУточняющимТекстом(Константы[МетаданныеКонстанты.Имя], ИнформацияОбОшибке());
		КонецПопытки;
	КонецЦикла;
	
	// 2) Независимые регистры сведений
	Для каждого МДРегистра Из Метаданные.РегистрыСведений Цикл
		ПолноеИмяОбъектаМД = МДРегистра.ПолноеИмя();
		...
		Попытка
			ВыгрузкаЗагрузкаДанныхПереопределяемый.ПередВыгрузкойДанных(НаборЗаписей);
			ЗаписатьXML(Запись, НаборЗаписей);
		Исключение
			ВызватьИсключениеСУточняющимТекстом(МенеджерТипа, ИнформацияОбОшибке());
		КонецПопытки;
	КонецЦикла;
	...
	Для каждого ВидОбъекта Из ВидыОбъектов Цикл
		...
		Для каждого МДОбъекта Из МетаданныеКоллекция Цикл
			ПолноеИмяОбъектаМД = МДОбъекта.ПолноеИмя();
			...
			|ИЗ
			|	" + ПолноеИмяОбъектаМД + " КАК _XMLВыгрузка_Таблица";
			РезультатЗапроса = Запрос.Выполнить();
			Выборка = РезультатЗапроса.Выбрать();
			Пока Выборка.Следующий() Цикл
				Объект = Выборка.Ссылка.ПолучитьОбъект();
				Попытка
					ВыгрузкаЗагрузкаДанныхПереопределяемый.ПередВыгрузкойДанных(Объект);
					ЗаписатьXML(Запись, Объект);
				Исключение
					ВызватьИсключениеСУточняющимТекстом(ПолноеИмяОбъектаМД, ИнформацияОбОшибке(), "Ссылка: " + Выборка.Ссылка);
				КонецПопытки;
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
	
	// 4) Регистры кроме независимых регистров сведений и последовательности
		...
		Для каждого МДРегистра Из МетаданныеКоллекция Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				Попытка
					...
					ЗаписатьXML(Запись, НаборЗаписей);
				Исключение
					ВызватьИсключениеСУточняющимТекстом(МенеджерТипа, ИнформацияОбОшибке(), "Регистратор: " + Выборка.Регистратор);
				КонецПопытки;
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
...
Процедура ПрочитатьДанныеОбластиИзXML(Знач Чтение) Экспорт
	...
	Пока Чтение.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
		...
		Данные.ДополнительныеСвойства.Вставить("НеПроверятьДатыЗапретаИзмененияДанных", Истина);
...
Процедура ЗаписатьИнформациюОВыгрузкеВXML(КаталогВыгрузки, БезДеленияПоФайлам)
	...
	ТипDumpInfo = ФабрикаXDTO.Тип("http://v8.1c.ru/misc/datadump", "DumpInfo");
	ТипConfigurationInfo = ФабрикаXDTO.Тип("http://v8.1c.ru/misc/datadump", "ConfigurationInfo");
	
	ИнформацияОВыгрузке = ФабрикаXDTO.Создать(ТипDumpInfo);
	Если БезДеленияПоФайлам Тогда
		ИнформацияОВыгрузке.Type = "ServiceToBox";
	Иначе
		ИнформацияОВыгрузке.Type = "BoxToService";
	КонецЕсли;
	ИнформацияОВыгрузке.Created = ТекущаяДата();
...
Функция ПолучитьМассивТиповПоставляемыхСправочников()
	...
	Для каждого СтрокаСоответствия Из ТаблицаСоответствия Цикл
		Если НЕ Справочники.ТипВсеСсылки().СодержитТип(СтрокаСоответствия.ТипОбщихДанных) Тогда
			Продолжить;
		КонецЕсли;
		
		Если СтрокаСоответствия.ТипРазделенныхДанных = Неопределено Тогда
			Тип = СтрокаСоответствия.ТипОбщихДанных;
		Иначе
			Тип = СтрокаСоответствия.ТипРазделенныхДанных;
		КонецЕсли;
		Типы.Добавить(Тип);
	КонецЦикла;
...
Процедура ЗаписатьВXML(Значение, МетаданныеЗначения, Размер, Сериализатор, ПараметрыЗаписи_ЗаменяемыеТипы,
	...
	ВыгрузкаЗагрузкаДанныхПереопределяемый.ПередВыгрузкойДанных(Значение);
...
Функция ПодготовитьСловарьЗамен()
	...
	Для каждого СтрокаСоответствия Из ОбщегоНазначения.ПолучитьТаблицуСоответствияРазделенныхДанныхОбщим() Цикл
		...
		Если СтрокаСоответствия.ТипРазделенныхДанных = Неопределено Тогда
			...
			ОписаниеТипаЗамены.Тип = СтрокаСоответствия.ТипОбщихДанных;
		Иначе
			ОписаниеТипаЗамены.ОбщиеДанные = Ложь;
			ОписаниеТипаЗамены.Тип = СтрокаСоответствия.ТипРазделенныхДанных;
		КонецЕсли;
		...
		МетаданныеОбщегоТипа = Метаданные.НайтиПоТипу(СтрокаСоответствия.ТипОбщихДанных);
...
Процедура ДобавитьФрагментВСловарьЗамен(Знач СловарьЗамен, Знач Фрагмент)
	...
	Для каждого СтрокаФрагмента Из Фрагмент Цикл
		...
		Для каждого КлючИЗначение Из СтрокаСловаря.СоответствиеСсылок Цикл
			ИсходнаяСсылкаXML = СериализаторXDTO.XMLСтрока(КлючИЗначение.Значение.УникальныйИдентификатор());
			НайденнаяСсылкаXML = СериализаторXDTO.XMLСтрока(КлючИЗначение.Ключ.УникальныйИдентификатор());
...
Процедура ЗагрузитьДанныеСПодменойСсылок(Знач КаталогВыгрузки, Знач РежимОтладки)
	
	СловарьЗамен = ПодготовитьСловарьЗамен();
	
	ФрагментСловаряЗамен = ПодготовитьФрагментСловаряЗамен();
	ВыгрузкаЗагрузкаДанныхПереопределяемый.ДополнитьСловарьЗаменПоКаталогуВыгрузки(ФрагментСловаряЗамен, КаталогВыгрузки);
	...
	Для каждого ОписаниеТипаЗамены Из СловарьЗамен Цикл
		Если ОписаниеТипаЗамены.ОбщиеДанные Тогда
			Продолжить;
		КонецЕсли;
		
		СтандартныеПодсистемыВызовСервераПереопределяемый.ПодобратьПоставляемыеДанныеИзКлассификатора(ОписаниеТипаЗамены.ОбщиеСсылки);
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗагрузитьДанныеЧерезXDTOСериализатор(Знач КаталогВыгрузки, Знач МаскаПоиска = Неопределено, Знач РежимОтладки = Ложь)
			...
			Для каждого ФайлДанных Из ВсеФайлыДанных Цикл
				...
				Пока Чтение.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
					...
					Если НЕ ЭтоПланОбмена Тогда
						Данные.ОбменДанными.Загрузка = Истина;
					КонецЕсли;
					ВыгрузкаЗагрузкаДанныхПереопределяемый.ПередЗагрузкойДанных(Данные);
					Данные.Записать();
				КонецЦикла;
			КонецЦикла;
...
Функция ЗаписатьПользователяВXDTO(Знач Пользователь, Знач СохранятьПароль = Ложь, Знач СохранятьРазделение = Ложь)
	...
	ТипInfoBaseUser = ФабрикаXDTO.Тип("http://v8.1c.ru/misc/datadump", "InfoBaseUser");
	ТипUserRoles    = ФабрикаXDTO.Тип("http://v8.1c.ru/misc/datadump", "UserRoles");
...
Функция СоздатьПользователейИзXML(Знач КаталогВыгрузки)
	...
	Пока Чтение.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
		...
		ОбработатьРолиПользователяПриЗагрузкиИзДругойМодели(Пользователь);
	...
	Если ТаблицаПользователей.Количество() > 0 Тогда
		...
		Иначе
			...
			Для каждого СтрокаПользователя Из ТаблицаПользователей Цикл
				Пользователи.ЗаписатьПользователяИнформационнойБазы(СтрокаПользователя.Пользователь);
...
Процедура ОбработатьРолиПользователяПриЗагрузкиИзДругойМодели(Пользователь)
	
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		// Удалим недоступные роли 
		НедоступныеРоли = 
			ПользователиСерверПовтИсп.НедоступныеРолиПоТипуПользователя(Перечисления.ТипыПользователей.ПользовательОбластиДанных);
		Для каждого Роль Из НедоступныеРоли Цикл
			Если Пользователь.Роли.Содержит(Роль) Тогда
				Пользователь.Роли.Удалить(Роль);
			КонецЕсли;
		КонецЦикла;
	Иначе
		// Добавим АдминистраторСистемы к роли ПолныеПрава
...
Процедура ВызватьИсключениеСУточняющимТекстом(Знач ОбъектЗаписи, Знач ИнформацияОбОшибке, Знач ТекстДополнительно = "")
	
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Недопустимые символы в объекте %1.
              |'"), "" + ОбъектЗаписи);
	Если ТекстДополнительно <> "" Тогда
		ТекстОшибки = ТекстОшибки + ТекстДополнительно + Символы.ПС;
	КонецЕсли;
	ЗаписьЖурналаРегистрации(НСтр("ru = 'Выгрузка данных'"),
		УровеньЖурналаРегистрации.Ошибка, , ,
