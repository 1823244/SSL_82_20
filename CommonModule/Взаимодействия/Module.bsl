...
Процедура ПриЗаписиПредметаИзФормы(Предмет, Взаимодействие, Отказ) Экспорт
	...
	Если ЗначениеЗаполнено(СтарыйПредмет)
		...
		МассивЗаменить = ВзаимодействияВызовСервера.ПолучитьВзаимодействияИзЦепочки(СтарыйПредмет, Взаимодействие);
	Иначе
	...
	Иначе
		...
		Для каждого ЭлементМассиваОписания Из МассивОписанийКонтактов Цикл
			Если ЭлементМассиваОписания.Имя = ИмяМетаданныхКонтакта Тогда
				...
				Если ЭлементМассиваОписания.ЕстьВладелец Тогда
					...
					ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(
						Форма["Список_" + ЭлементМассиваОписания.Имя].Отбор,"Владелец",Контакт.Владелец,,,Истина);
				КонецЕсли;
			ИначеЕсли ЭлементМассиваОписания.ИмяВладельца = ИмяМетаданныхКонтакта Тогда
				...
				ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(Форма["Список_" + ЭлементМассиваОписания.Имя].Отбор,
					                                                 "Владелец",Контакт,,,Истина);
			КонецЕсли;
		КонецЦикла;
...
Процедура ЗаполнитьКонтактыДляВстречи(Контакты, Участники) Экспорт
	...
	Для Каждого ЭлементМассива Из Контакты Цикл
		...
		ДозаполнитьПоляКонтактов(НоваяСтрока.Контакт, НоваяСтрока.ПредставлениеКонтакта, НоваяСтрока.КакСвязаться);
...
//
// Возвращаемое значение:
//   Произовльное   - значение свойства метаданных
//
...
//
//Параметры
//  УчетнаяЗапись - СправочникСсылка.УчетныеЗаписиЭлектроннойПочты - учетная запись, для которой определяется ответственный
//
// Возвращаемое значение:
//   СправочникСсылка.Пользователи   - ответственный за обработку электронных писем
//
Функция ПолучитьОтвественногоЗаОбработкуПисьмаПоУмолчанию(УчетнаяЗапись) Экспорт
...
//Регистрирует обработчики обновления ИБ для подсистемы Взазимодействий
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "1.1.2.4";
	Обработчик.Процедура = "Взаимодействия.ЗаполнитьКонстантыИспользованияВзаимодействий_1_2_1_4";
...
//Перещает письма в предопределенные папки
//
...
Процедура ОбновитьХранениеОтветственныйУчетнаяЗапись_1_2_1_4() Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	|	УчетныеЗаписиЭлектроннойПочты.Ссылка,
	|	УчетныеЗаписиЭлектроннойПочты.ОтветственныйЗаОбработкуПисем
	|ИЗ
	|	Справочник.УчетныеЗаписиЭлектроннойПочты КАК УчетныеЗаписиЭлектроннойПочты
	|ГДЕ
	|	УчетныеЗаписиЭлектроннойПочты.ОтветственныйЗаОбработкуПисем <> ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)";
	...
	Пока Выборка.Следующий() Цикл
		...
		МенеджерЗаписи.ОтветственныйЗаОбработкуПисем     = Выборка.ОтветственныйЗаОбработкуПисем;
		МенеджерЗаписи.ОтветственныйЗаВедениеПапок       = Выборка.ОтветственныйЗаОбработкуПисем;
...
//Определяет оценочный размер исходящих писем.
//
Процедура ЗаполнитьКонстантыИспользованияВзаимодействий_1_2_1_4() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	УдалитьИспользоватьВзаимодействия.Значение КАК ИспользоватьВзаимодействия
	|ИЗ
	|	Константа.УдалитьИспользоватьВзаимодействия КАК УдалитьИспользоватьВзаимодействия";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий();
	
	Если Выборка.ИспользоватьВзаимодействия Тогда
...
//////////////////////////////////////////////////////////////////////////////////
//   Работа с элементами и реквзитами форм списков и документов
...
Процедура ДобавитьСтраницыФормыПодбораКонтактов(Форма) Экспорт
	...
	Для каждого ЭлементМассиваОписания Из МассивОписанийВозможныхКонтактов Цикл
		...
		Если ЭлементМассиваОписания.ЕстьВладелец Тогда
			...
			ЭлементОтбора = Форма["Список_" + ЭлементМассиваОписания.Имя].Отбор.Элементы.Добавить(
...
Процедура СоздатьОтборПоТипуСогласноФО(Список) Экспорт
	...
	ГруппаОтбора = ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(
		Список.Отбор.Элементы,"Отбор по типу согласно ФО", ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИ);
...
// Определяет необходимость отображнения для адресной книги и форм выбора контакта групп пользователей
//
// Параметры
...
// Добравляет строку с данными исходного письма
// Параметры
...
// Создает атрибут элемента HTML и устанавливает его текствое содержимое
//
// Параметры
//  ЭлементHTML  - ЭлементHTML - элемент для которого устанавливается атрибут
//  Имя  - Строка - имя атрибута HTML
//  ТекстовоеСодержимое  - Строка - текствое содержимое атрибута
//
...
// Параметры
//  ЭлементРодитель  - ЭлементHTML - элемент,к которому будет добавлен дочерний элемент
//  Текст  - Строка - содержимое текствого
//  Атрибуты  - Соответствие - ключ содержит имя атрибута, значение текстовое содержимое
//
// Возвращаемое значение:
//   ЭлементHTML   - добавленный элемент
...
Процедура СохранитьНастройкуТекущегоПользователя(КлючОбъекта, КлючНастроек = Неопределено, Значение)
...
// Сохраняет настройку
//
Процедура СохранитьНастройку(КлючОбъекта, КлючНастроек = Неопределено, Значение) Экспорт
	
	СохранитьНастройкуТекущегоПользователя(КлючОбъекта, КлючНастроек, Значение);
...
Процедура СохранитьНастройкуРаботаСПочтой(Значение) Экспорт
	...
	СохранитьНастройкуТекущегоПользователя("РаботаСПочтой", "НастройкиПользователя", Значение);
...
// Проверяет, является ли текущий пользователь ответсвенным за ведение папок для учетной записи
//
// Параметры
...
Процедура УстановитьРодителяУПапки(Папка, НовыйРодитель, НеЗаписыватьПапку = Ложь) Экспорт
	...
	Если НЕ НовыйРодитель.Пустая()Тогда
		ЗначенияРеквизитовПапки = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(
...
Процедура УстановитьПапкуДляМассиваПисем(МассивПисем,Папка) Экспорт
	...
	ЗначенияРеквизитовПапки = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Папка, "ПредопределеннаяПапка,Наименование");
...
// Параметры
//  Письмо  - ДокументСсылка.ЭлектронноеПисьмоВходящее,
//            ДокументСсылка.ЭлектронноеПисьмоИсходящее - письмо для которого определяестя папка
//
// Возвращаемое значение:
