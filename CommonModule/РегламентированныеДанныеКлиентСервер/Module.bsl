...
Функция КодПоОКПОСоответствуетТребованиям(КодПоОКПО,ЭтоЮрЛицо,ТекстСообщения = "") Экспорт
	...
	Если СоответствуетТребованиям Тогда
		...
		ДлинаКода = ?(ЭтоЮрЛицо,8,10);
		
		Для инд = 1 По (ДлинаКода-1) Цикл
			
			КонтрольнаяСумма = КонтрольнаяСумма + Сред(КодПоОКПО,инд,1) * инд;
		...
		Если КонтрольнаяЦифра = 10 Тогда
			...
			Для инд = 1 По (ДлинаКода-1) Цикл
				...
				КонтрольнаяСумма = КонтрольнаяСумма + Сред(КодПоОКПО,инд,1) * (инд + 2);
		...
		Если Число(Сред(КодПоОКПО,ДлинаКода,1)) <> КонтрольнаяЦифра Тогда
