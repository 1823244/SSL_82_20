...
// Позволяет установить описание варианта отчета. Для использования из модуля менеджера отчета.
// 
// Параметры:
//   Настройки (Структура) Передается "как есть" из первого параметра процедуры НастройкиОтчета.
//   Ключ      (Строка)    Имя варианта отчета. Если необходимо зарегистрировать сам отчет, а не вариант
//                         (например, если отчет не на СКД), то в этот параметр необходимо передать "_СсылкаНаОтчет".
//   Описание  (Строка)    Описание варианта отчета. Локализуем.
//
// Например: 
//   Процедура НастройкиОтчета(Настройки) Экспорт
//       ВариантыОтчетов.УстановитьОписаниеВариантаВДопНастройках(Настройки, "ПОСчету50", НСтр("ru = 'Кассовые операции за текущий месяц'"));
//   КонецПроцедуры
//
Процедура УстановитьОписаниеВариантаВДопНастройках(Настройки, Ключ, Описание) Экспорт
	
	Настройки.ОписаниеВариантов.Вставить(Ключ, Описание);
...
//	
//	Процедура НастройкиОтчета(Настройки) Экспорт
//		ВариантыОтчетов.ДобавитьПредопределенныйВариантВДопНастройках(Настройки, "Отчет.ОСВ\ПОСчету50", НСтр("ru = 'Кассовые операции'"), Новый Структура("Счет", 50));
//	КонецПроцедуры
//
			...
			Если НЕ ПравоДоступа("Просмотр", ОтчетМД) Тогда
				Если ЧерезИсключение Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Недостаточно прав доступа для работы с отчетом %1'"),
...
Функция ОбработкаПредопределенныхВариантов(ИзмененныеОбъектыДанных = Неопределено) Экспорт
		...
		Для каждого Вариант из ВариантыДляОбновления Цикл
			...
			Для каждого Подсистема из МассивПодсистем Цикл
				...
				Если НеИспользоватьПодсистемуОтчета.Количество() = 0 тогда 
					СтрокаПодсистемы = ВариантОбъект.Подсистемы.Найти(Подсистема.ПутьКПодсистеме, "Подсистема");
	...
	Исключение	
		...
		ЗаписьЖурналаРегистрации(
			...
			,
			НСТр("ru = 'В процессе обновления вариантов возникла ошибка:'")
			+ Символы.ПС
