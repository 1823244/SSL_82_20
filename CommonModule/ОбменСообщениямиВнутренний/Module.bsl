...
// Обработчик события ПередОтправкойДанных подсистемы Обмена данными
// Описание обработчика см. ОбщийМодуль.ОбменДаннымиПереопределяемый.ПередОтправкойДанных()
//
...
// Обработчик события ПередПолучениемДанных подсистемы Обмена данными
// Описание обработчика см. ОбщийМодуль.ОбменДаннымиПереопределяемый.ПередПолучениемДанных()
//
...
Процедура ВыполнитьПодключениеКонечнойТочкиНаСторонеПолучателя(Отказ, Код, Наименование, НастройкиПодключенияПолучателя) Экспорт
	...
	Попытка
		...
		Если Не РегламентноеЗадание.Использование Тогда
			РегламентныеЗаданияСервер.УстановитьИспользованиеРегламентногоЗадания(Строка(РегламентноеЗадание.УникальныйИдентификатор), Истина);
		КонецЕсли;
...
Функция КодЭтогоУзла() Экспорт
	...
	Возврат ОбщегоНазначения.ПолучитьЗначениеРеквизита(ЭтотУзел(), "Код");
...
Функция НаименованиеЭтогоУзла() Экспорт
	...
	Возврат ОбщегоНазначения.ПолучитьЗначениеРеквизита(ЭтотУзел(), "Наименование");
...
Процедура ВыполнитьПодключениеКонечнойТочкиНаСторонеОтправителя(Отказ,
	...
	Если Не УзелКонечнойТочки.Пустая() Тогда
		...
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщенияОбОшибке, ОбщегоНазначения.ПолучитьЗначениеРеквизита(УзелКонечнойТочки, "Наименование"));
		ЗаписьЖурналаРегистрации(СобытиеЖурналаРегистрацииПодключениеКонечнойТочки(), УровеньЖурналаРегистрации.Ошибка,,, СтрокаСообщенияОбОшибке);
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		ПараметрыЭтойТочки = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(ЭтотУзел(), "Код, Наименование");
