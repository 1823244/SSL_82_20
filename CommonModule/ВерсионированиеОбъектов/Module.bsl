...
Процедура ЗаписатьНастройкуВерсионированияПоОбъекту(ИмяОбъектаМетаданных, ВариантВерсионирования) Экспорт
	...
	ВариантМВ = РегистрыСведений.НастройкиВерсионированияОбъектов.СоздатьМенеджерЗаписи();
	ВариантМВ.ТипОбъекта = ИмяОбъектаМетаданных;
...
//
// Параметры
//  Ссылка               - Ссылка - ссылка на объект ИБ;
//  НомерПоследнейВерсии - Число - номер последней записанной версии объекта;
//  ХранилищеДанных      - ХранилищеЗначения - хранилище с объектом ИБ (сериализованным в Fast InfoSet);
//  Комментарий          - Строка - комментарий к записываемой версии.
//
Процедура ЗаписатьВерсиюОбъекта(знач Ссылка,
                                знач НомерПоследнейВерсии,
...
//
// Параметры:
//  Ссылка - Ссылка - ссылка на объект информационной базы.
//
// Возвращаемое значение:
//  Число - номер версии объекта.
//
Функция ПолучитьНомерПоследнейВерсии(Ссылка) Экспорт
...
//
// Параметры
//  ПолноеИмя - строка - полный путь к объекту метаданных. Например, "Справочник.Номенклатура".
//
// Возвращаемое значение:
//  Перечисление.ВариантыВерсионированияОбъектов.
//
Функция ПолучитьВариантВерсионирования(ПолноеИмя) Экспорт
	
	Возврат ПолучитьФункциональнуюОпцию("ВариантыВерсионированияОбъектов",
		Новый Структура("ТипВерсионируемогоОбъекта", ПолноеИмя));
...
//
Функция ПолучитьНастройкуВерсионированияПоУмолчанию(ПолноеИмя) Экспорт
...
//
Функция ПолучитьИнформациюПоВерсии(знач Ссылка, знач НомерВерсии) Экспорт
	...
	Если ВерсияОбъекта = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не удалось получить версию объекта из информационной базы. Выполнение операции прервано.'");
	КонецЕсли;
