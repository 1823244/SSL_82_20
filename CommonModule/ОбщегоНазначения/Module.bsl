...
// Возвращает подробную информацию о конфигурации (свойство метаданных 
// конфигурации)
//
// Возвращаемое значение: 
// Строка.
// Строка содержащая подробную информацию о конфигурации.
// 
Функция ПолучитьПодробнуюИнформациюКонфигурации() Экспорт
	
	Возврат Метаданные.ПодробнаяИнформация;
...
// - Подсистемы (вручную через предопределенные элементы)
// - Роли
// - ПланыОбмена
// - Справочники
// - Документы
...
Функция ОбъектМетаданныхПоИдентификатору(Идентификатор) Экспорт
	...
	Если НЕ Выгрузка[0].Используется Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при выполнении функции ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору().
			           |
			           |Идентификатор ""%1""
			           |найден в справочнике ""Идентификаторы объектов метаданных"",
			           |но значение реквизита ""Используется"" установлено Ложь.
			           |
			           |Если справочник не был обновлен при обновлении информационной базы,
			           |следует выполнить обновление вручную:
			           |""Все функции"" -> ""Справочник. Идентификаторы объектов метаданных"" ->
...
// ПредустановленныеИдентификаторыОбъектовМетаданных() для описания предустановленых идентификаторов
// 
// Параметры:
//   Идентификаторы       - передается в процедуру подсистемой БазоваяФункциональность
//   ИдентификаторВСтроке - Строка - уникальный идентификатор, назначаемый объекту метаданных
//   ОбъектМетаданных     - ОбъектМетаданных, для которого предустанавливается идентификатор
// 
Процедура ДобавитьИдентификатор(Идентификаторы, ИдентификаторВСтроке, ОбъектМетаданных) Экспорт
	
	Попытка
		Идентификатор = Справочники.ИдентификаторыОбъектовМетаданных.ПолучитьСсылку(Новый УникальныйИдентификатор(ИдентификаторВСтроке));
	Исключение
	...
	Если ТипЗнч(ОбъектМетаданных) <> Тип("ОбъектМетаданных") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка в процедуре ОбщегоНазначенияПереопределяемый.Заполнить-
			           ...
			           |
			           |Для идентификатора ""%1"" не заполнен объект метаданных.'"),
			ИдентификаторВСтроке);
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Идентификатор) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка в процедуре ОбщегоНазначенияПереопределяемый.Заполнить-
			           |ПредустановленныеИдентификаторыОбъектовМетаданных()
			           |
			           |Ошбика в строке идентификатора ""%1"",
			           |заданного для объекта метаданных
			           |""%2"".'"),
			ИдентификаторВСтроке,
			ПолноеИмя);
	КонецЕсли;
	
	Если Идентификаторы.Найти(ПолноеИмя, "ПолноеИмя") <> Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка в процедуре ОбщегоНазначенияПереопределяемый.Заполнить-
			           |ПредустановленныеИдентификаторыОбъектовМетаданных()
			           |
			           |Для объекта метаданных ""%1""
			           |идентификатор уже задан.'"),
			ПолноеИмя);
	КонецЕсли;
	
	Если Идентификаторы.Найти(Идентификатор, "Идентификатор") <> Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка в процедуре ОбщегоНазначенияПереопределяемый.Заполнить-
			           |ПредустановленныеИдентификаторыОбъектовМетаданных()
			           |
			           |Идентификатор ""%1""  уже задан
			           |для объекта метаданных ""%2"".'"),
			ИдентификаторВСтроке,
			ПолноеИмя);
	КонецЕсли;
	
	ОписаниеИдентификатора = Идентификаторы.Добавить();
	ОписаниеИдентификатора.ПолноеИмя     = ПолноеИмя;
...
Процедура ЗаблокироватьИБ() Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.РазделениеВключено() 
		...
		КонецЕсли;
	Иначе
		СтандартныеПодсистемыПереопределяемый.ЗаблокироватьТекущуюОбластьДанных();
	КонецЕсли;
...
Процедура УстановитьПараметрыРазделенияИнформационныйБазы(Знач ВключитьРазделение = Ложь) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	Константы.ИспользоватьРазделениеПоОбластямДанных.Установить(ВключитьРазделение);
...
//
Функция ПолучитьВерсииИнтерфейса(Знач ПараметрыПодключения, Знач ИмяИнтерфейса) Экспорт
...
Функция ПолучитьВерсииИнтерфейсаВКэш(Знач ПараметрыПодключения, Знач ИмяИнтерфейса)
	...
	СтандартныеПодсистемыПереопределяемый.ПреобразоватьПараметрыПодключенияКСервису(ПараметрыПодключения, ИмяИнтерфейса);
	...
	ПроксиВерсионирования = ОбщегоНазначенияПовтИсп.ПолучитьWSПрокси(АдресСервиса, "http://www.1c.ru/SaaS/1.0/WS",
		"InterfaceVersion", , ИмяПользователя, ПарольПользователя);
