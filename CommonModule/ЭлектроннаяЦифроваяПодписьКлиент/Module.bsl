...
Функция ПолучитьМассивСтруктурСертификатов(знач ТолькоЛичные) Экспорт
	...
	// здесь ТекущаяДата() используется для проверки на истекание срока сертификатов на клиентском компьютере - перенести ее на сервер в принципе невозможно
...
Функция ПолучитьМенеджерКриптографии(Отказ = Ложь) Экспорт
	...
	Если ПустаяСтрока(ПровайдерЭЦП) Тогда
		СтрокаОшибки = НСтр("ru='Не указан провайдер ЭЦП. В настройках криптографии укажите провайдера ЭЦП, тип провайдера, алгоритмы подписи и хеширования.'");
