...
// Возвращает структуру для удобства описания начальных профилей.
//
...
////////////////////////////////////////////////////////////////////////////////
// Процедуры и функции, используемые для обновления вторичных данных
...
//  Вызывается из УправлениеДоступомСобытия.ЗаписатьГруппыДоступа(), но может быть вызвана
...
Процедура ЗаписатьГруппыИВидыДоступа(Знач Объект) Экспорт
	...
	Если ТипЗнч(СсылкаНаОбъект) = Тип("СправочникСсылка.Пользователи") ИЛИ
		...
		ОбновитьПодчиненныхПользователейГруппДоступа();
...
// 
// Параметры:
//  МассивПользователей - Массив элементов СправочникСсылка.Пользователи, СправочникСсылка.ВнешниеПользователи.
//
Процедура ОбновитьРолиПользователей(Знач МассивПользователей) Экспорт
	...
	// Все пользователи, внешние пользователи и администраторы
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	Пользователи.Ссылка КАК Пользователь
...
Процедура ОбновитьПоставляемыеПрофилиИГруппыДоступа(ТолькоНеИзмененные = Истина, ОбновлятьГруппыДоступа = Истина, УдаляемыеВидыДоступаБезНастройки = Истина) Экспорт
	...
	Для каждого ОписаниеПрофиля Из ОписанияПрофилей Цикл
		...
		Если ОбновлятьГруппыДоступа Тогда
			ГруппыДоступаПрофиляОбновлены = ОбновитьГруппыДоступаПрофиля(Профиль, УдаляемыеВидыДоступаБезНастройки);
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "УправлениеДоступом.ПриИзмененииКонстантыДобавлятьРуководителямДоступПодчиненных";
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "1.1.3.4";
	Обработчик.Процедура = "УправлениеДоступом.ОбновитьПодчиненностьПользователей_1_1";
...
// Только для внутреннего использования. Вызывается из обработчика ПриЗаписи группы доступа.
Процедура ОбновитьРолиПользователейПриИзмененииГруппыДоступа(ГруппаДоступа, СтарыеУчастникиГруппыДоступа) Экспорт
	...
	ОбновитьРолиПользователей(СтарыеИНовыеПользователи);
...
Процедура ПриИзмененииКонстантыДобавлятьРуководителямДоступПодчиненных() Экспорт
	...
	ЗаписатьГруппыИВидыДоступа(Справочники.ГруппыПользователей.ВсеПользователи);
	ЗаписатьГруппыИВидыДоступа(Справочники.ГруппыВнешнихПользователей.ВсеВнешниеПользователи);
...
Процедура ОбновитьПодчиненностьПользователей_1_1_2_3() Экспорт
	...
	Константы.ДобавлятьРуководителямДоступПодчиненных.Установить(Ложь);
	Константы.ДобавлятьРуководителямДоступПодчиненных.Установить(Истина);
