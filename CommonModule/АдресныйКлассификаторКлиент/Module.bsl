		...
		Иначе
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Обновление не требуется.
						           ...
						           |при сравнении с версией классификатора от %1.'"),
						Формат(Результат.ВерсияПоследнегоОбновленияКЛАДР, "ДЛФ=D"));
			Предупреждение(ТекстПредупреждения, , НСтр("ru = 'Адресный классификатор'"));
		КонецЕсли;
	Иначе
		Предупреждение(НСтр("ru = 'Адресный классификатор не заполнен.'"), , НСтр("ru = 'Адресный классификатор'"));
	КонецЕсли;
