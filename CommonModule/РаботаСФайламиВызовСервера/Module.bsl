		...
		Если ТекущаяВерсияРазмер <= МаксРазмерФайла / 2 Тогда
			...
			Если ТекущаяВерсияТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
				Если НЕ ТекущаяВерсияТом.Пустая() Тогда
					ПолныйПуть = ФайловыеФункции.ПолныйПутьТома(ТекущаяВерсияТом) + ТекущаяВерсияПутьКФайлу; 
...
Процедура ПолучитьПерсональныеНастройкиФайловыхФункций(Настройки) Экспорт
	...
	Настройки.Вставить("ЭтоПолноправныйПользователь", Пользователи.ЭтоПолноправныйПользователь());
...
//
// Параметры
//  ИмяТекущее - Строка - текущее имя пользователя
//  ИмяУстанавливаемое - Строка - новое имя пользователя
...
Процедура УдалитьФайлыПриПолученииДанныхФайла(Ссылка, ПутьСПодкаталогом) Экспорт
	
	Если РаботаСФайламиСобытия.ЭтоЭлементРаботаСФайлами(Ссылка) Тогда
