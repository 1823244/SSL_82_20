...
Функция ПодготовитьПараметрыИВыполнитьРассылку(Рассылка, ПараметрыЖурнала = Неопределено, ПредварительныеНастройки = Неопределено) Экспорт
	...
	Если НЕ ПараметрыЖурнала.Свойство("ИмяСобытия") Тогда
		ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ru = 'Рассылка отчетов. Запуск по требованию'"));
	КонецЕсли;
	...
	// Проверки отмеченных спсобов доставки
...
//       |- Форматы (Массив) из (ПеречислениеСсылка.ФорматыСохраненияОтчетов) Форматы, 
//                           в которых необходимо сохранить и отправить отчет
//
...
//       |- ПараметрыПисьма (Структура)    Параметры сообщения, которые будут переданы подсистеме "РаботаСПочтовымиСообщениями" напрямую.
...
//               |- Значение (...) - АдресФайлаВложения (Текст) / ДвоичныеДанные (ДвоичныеДанные) / ИнтернетПочтовоеСообщение 
//           |- АдресОтвета (Строка) - e-mail адрес ответа
//
//     --- Подчиненность значений различных параметров ---
//
...
Процедура ДобавитьЭлементВТаблицуТиповПолучателей(ТаблицаТипов, ДоступныеТипы, Настройки) Экспорт
	...
	// Регистрирация использования основного типа
	...
	Если Настройки.Свойство("ДополнительныйТип") Тогда
		...
		// Регистрирация использования дополнительного типа
...
//       |- МассивРассылок           (Массив) из (СправочникСсылка.Рассылки.Отчетов)
//       |- ПредварительныеНастройки (Структура) см. функцию ПодготовитьПараметрыИВыполнитьРассылку()
//   АдресРезультата     (Строка)    Адрес, по которому будет размещен результат
//       |- (Структура) 
// 
...
Процедура ВыполнитьРассылкуПоРасписанию(Рассылка) Экспорт
	...
	ПараметрыЖурнала.ИмяСобытия = НСтр("ru = 'Рассылка отчетов. Запуск по расписанию'");
...
Функция СформироватьСписокПолучателейРассылки(ПараметрыЖурнала, Рассылка) Экспорт
	...
	Иначе
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Пользователи", ПолучателиМД.Имя);
...
// 
// Параметры:
//   ПараметрыОтчета (Структура) - Начальные параметры
//       - Отчет и его Настройки (см. Возвращаемое значение)
//   ПерсонализацияВозможна (Булево) - Возможность персонализации отчета
//       Истина - отчет может быть персонализирован
//       Ложь   - отчет не может быть персонализирован
//   УникальныйИдентификаторФормы (УникальныйИдентификатор) 
//       - Если заполнен, то схема компоновки данных будет помещена во временное хранилище вплоть до закрытия формы
// 
...
//           | Отчет, который необходимо инициализировать
...
//
//       Общие параметры:
//       |- Имя        (Строка)           - Имя объекта метаданных отчета
//       |- ЭтоВариант (Булево)           - Истина - используется справочник "ВариантыОтчетов", 
//                                          Ложь   - "ДополнительныеОтчетыИОбработки"
//       |- СКД        (Булево)           - Истина - отчет принадлежит Системе Компоновки Данных, 
//                                          Ложь   - это произвольный отчет
//       |- Метаданные (ОбъектМетаданных) - Метаданные отчета
//       |- Объект     (ОтчетОбъект.*)    - Объект отчета 
//       
//       Параметры, которые заполняеются для СКД отчетов:
...
//                                           используется только если передано значение УникальныйИдентификаторФормы
//       |- НастройкиКД           (НастройкиКомпоновкиДанных) - ...
//       
//       Параметры, которые заполняеются для произвольных отчетов:
//       |- ДоступныеРеквизиты (Структура) - Набор доступных реквизитов
//           |- Ключ     (Строка)    - Имя реквизита
//           |- Значение (Структура) - Представление и тип реквизита
...
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ПерсонализацияВозможна, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	// Безовые проверки и настройки отчета
	...
	Если ПараметрыОтчета.ЭтоВариант Тогда
		...
		ПараметрыОтчета.Имя = СокрЛП(СтрЗаменить(ПараметрыОтчета.Отчет.КлючОбъекта, "Отчет.", ""));
		ПараметрыОтчета.Метаданные = Метаданные.Отчеты.Найти(ПараметрыОтчета.Имя);
		Если ПараметрыОтчета.Метаданные = Неопределено Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				НСтр("ru = 'Для варианта %1 отчет %2 не найден в системе'"), ,
		...
		ПараметрыОтчета.Объект = Отчеты[ПараметрыОтчета.Имя].Создать();
	...
	Иначе
		...
		Попытка
			РассылкаОтчетовПереопределяемый.ПодключитьДополнительныйОтчет(ПараметрыОтчета.Отчет, ПараметрыОтчета, Результат);
	...
	Если ПараметрыОтчета.СКД Тогда
		...
		Если ПараметрыОтчета.ЭтоВариант Тогда
			ПараметрыОтчета.НастройкиКД = ПараметрыОтчета.Отчет.ХранилищеЗначений.Получить();
	...
	Иначе // НЕ ПараметрыОтчета.СКД 
		...
		Если ЗначениеЗаполнено(ПараметрыОтчета.Настройки) Тогда
			...
			КонецЦикла; // Для Каждого КлючИЗначение Из ПараметрыОтчета.Настройки Цикл
			
		КонецЕсли; // ЗначениеЗаполнено(ПараметрыОтчета.Настройки)
		
	КонецЕсли; // Если ПараметрыОтчета.СКД Тогда
...
//       |- Выполнена (Булево) - Флаг того, что доставка выполнена как минимум одним из способов
//
Функция ВыполнитьДоставку(ПараметрыЖурнала, ПараметрыДоставки, Вложения) Экспорт
	...
	Если ПараметрыДоставки.ИспользоватьFTPРесурс Тогда
		...
		Попытка
			Прокси = Неопределено;
			РассылкаОтчетовПереопределяемый.НастройкиПроксиНаСервере(Прокси);
	...
	Если ПараметрыДоставки.ИспользоватьПапку Тогда
		...
		Попытка
			РассылкаОтчетовПереопределяемый.ВыполнитьДоставкуВПапку(ПараметрыДоставки, Вложения);
			Результат = Истина;
...
//       |- Метаданные (ОбъектМетаданных) Метаданные для привязки события журнала регистрации
//       |- Данные                    (*) Данные для привязки события журнала регистрации
//   УровеньЖурнала (УровеньЖурналаРегистрации) Уровень важности события
//   КомментарийСПараметрами (Строка) Комментарий, можно передавать с параметрами %1, %2, %3 и %4
//   ИнформацияОбОшибке         (...) Информация об ошибке, которая будет размещена
//       |- (ИнформацияОбОшибке)      в комментарии к записи журнала регистрации
//       |- (Строка)
//   Параметр1               (Строка) Для подстановки в комментарий вместо %1
//   Параметр2               (Строка) Для подстановки в комментарий вместо %2
//   Параметр3               (Строка) Для подстановки в комментарий вместо %3
//   Параметр4               (Строка) Для подстановки в комментарий вместо %4
//
Процедура ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурнала = Неопределено, КомментарийСПараметрами = "", 
	...
	Если ТипЗнч(ИнформацияОбОшибке) = Тип("ИнформацияОбОшибке") Тогда
		...
		ТекстДляПользователя = ТекстДляПользователя + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке);
	ИначеЕсли ТипЗнч(ИнформацияОбОшибке) = Тип("Строка") Тогда
...
Процедура СформироватьИСохранитьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ДеревоОтчетов, ПараметрыДоставки, ПолучательСсылка)
	...
	Для Каждого Формат Из ПараметрыОтчета.Форматы Цикл
		...
		// Передача управления в переопределяемый модуль
...
КонецПроцедуры // СформироватьИСохранитьОтчет()
...
Функция ЗаполнитьПараметрыРассылкиПараметрамиПоУмолчанию(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала)
	...
	Если ПараметрыДоставки.ИспользоватьПапку Тогда
		...
		Иначе
			ПравоДоступа = Истина;
			РассылкаОтчетовПереопределяемый.ПравоИзмененияПапкиИФайлов(ПараметрыДоставки.Папка, ПравоДоступа);
...
Функция ПривестиИмяФайла(ИсходноеИмяФайла)
	...
	Для Каждого КлюИЗначение Из СоответствиеЗамен Цикл
		Результат = СтрЗаменить(Результат, КлюИЗначение.Ключ, КлюИЗначение.Значение);
	КонецЦикла;
...
// Формирует представленеи способов доставки в соответствии с параметрами доставки
// 
// Параметры:
//   ПараметрыДоставки (Структура) - см. функцию ВыполнитьРассылку
// 
...
Процедура НайтиСвободноеИмяФайла(ПолноеИмяФайла)
	...
	Пока Файл.Существует() Цикл
		...
		Файл = Новый Файл(ПолноеИмяФайла);
	КонецЦикла;
КонецПроцедуры // НайтиСвободноеИмяФайла 

// Создает корневую строку дерева для получателя (в случае ее отсутствия) и заполнет ее свойства параметрами по умолчанию
// 
// Параметры:
...
КонецПроцедуры // СформироватьПредставлениеОтчетовДляПолучателя 
