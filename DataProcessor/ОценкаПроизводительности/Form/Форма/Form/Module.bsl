...
Процедура ПроизводительностьВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОткрытьФормуМодально("Обработка.ОценкаПроизводительности.Форма.ИсторияВыполнения", ПараметрыОткрытия);
...
Процедура ЭкспортДанных(Команда)
	...
	СохранитьФайлНаКлиент(АдресВХранилище, ИмяФайла);
...
Процедура ЗаполнитьОписание(ИмяФайлаОписания, Начало, Конец, МассивКО)
	...
	ЗаписьXML.ЗаписатьАтрибут("ExportDate", Строка(ТекущаяДатаСеанса()));
...
// Записывает файл из хранилища по указанному пути.
&НаКлиенте
Процедура СохранитьФайлНаКлиент(Знач Адрес, Знач ИмяФайла)
	
	#Если Не ВебКлиент Тогда
		
		ПолучитьФайл(Адрес, ИмяФайла, Ложь);
		Предупреждение(НСтр("ru = 'Экспорт завершен.'"));
		
	#Иначе
		
		Если ИмяФайла <> Неопределено Тогда
			ОписаниеПередаваемогоФайла = Новый ОписаниеПередаваемогоФайла;
			ОписаниеПередаваемогоФайла.Хранение = Адрес;
		...
		Иначе
			// Интерактивное получение с запросом имени файла
...
Процедура ОбментяьПриоритеты(КлючеваяОперация1, Приоритет1, КлючеваяОперация2, Приоритет2)
	...
	ОбъектКлючевойОперации = ОценкаПроизводительностиВызовСервераПолныеПрава.НачатьИзменениеОбъекта(КлючеваяОперация1);
	...
	ОценкаПроизводительностиВызовСервераПолныеПрава.ЗафиксироватьИзменениеОбъекта(ОбъектКлючевойОперации);
	
	ОбъектКлючевойОперации = ОценкаПроизводительностиВызовСервераПолныеПрава.НачатьИзменениеОбъекта(КлючеваяОперация2);
...
Процедура ИзменитьЦелевоеВремя(КлючеваяОперация, ЦелевоеВремя)
	...
	КлючеваяОперацияОбъект = ОценкаПроизводительностиВызовСервераПолныеПрава.НачатьИзменениеОбъекта(КлючеваяОперация);
	КлючеваяОперацияОбъект.ЦелевоеВремя = ЦелевоеВремя;
	ОценкаПроизводительностиВызовСервераПолныеПрава.ЗафиксироватьИзменениеОбъекта(КлючеваяОперацияОбъект);
