...
&НаКлиенте
Процедура ИзменитьРегламентноеЗаданиеВыполнить(Команда)
...
Процедура ОткрытьРасписаниеРегламентногоЗаданияВыполнить(Команда)
	...
	Иначе
		Диалог = Новый ДиалогРасписанияРегламентногоЗадания(
			РегламентныеЗаданияКлиент.ПолучитьРасписаниеРегламентногоЗадания(ТекущиеДанные.Идентификатор));
...
Процедура ОткрытьФоновоеЗадание()
	...
	Иначе
		...
		ОткрытьФормуМодально("Обработка.РегламентныеИФоновыеЗадания.Форма.ФоновоеЗадание", ПараметрыФормы);
	КонецЕсли;
...
Функция ВыполнитьРегламентноеЗаданиеВручнуюНаСервере(Знач ИдентификаторРегламентногоЗадания,
	...
	ЗапускВыполнен = РегламентныеЗаданияСервер.ВыполнитьРегламентноеЗаданиеВручную(
		ИдентификаторРегламентногоЗадания,
		МоментЗапуска,
		ИдентификаторФоновогоЗадания,
		МоментОкончания,
...
Процедура ДобавитьСкопироватьИзменитьРегламентноеЗадание(Знач Действие)
	...
	Иначе
		...
		ОткрытьФормуМодально("Обработка.РегламентныеИФоновыеЗадания.Форма.РегламентноеЗадание", ПараметрыФормы);
	КонецЕсли;
...
Процедура ОбновитьСтрокуТаблицыРегламентныхЗаданий(Строка, Задание);
	...
	СвойстваПоследнегоФоновогоЗадания = РегламентныеЗаданияСервер
...
Процедура ОбновитьТаблицуФоновыхЗаданий()
	...
	ТекущаяТаблица = РегламентныеЗаданияСервер.ПолучитьТаблицуСвойствФоновыхЗаданий(
	...
	Для каждого Задание ИЗ ТекущаяТаблица Цикл
		...
		Обновляемое.СообщенияПользователюИОписаниеИнформацииОбОшибке 
			= РегламентныеЗаданияСервер.СообщенияИОписанияОшибокФоновогоЗадания(
