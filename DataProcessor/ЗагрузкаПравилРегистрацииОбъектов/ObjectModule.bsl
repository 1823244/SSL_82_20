...
Перем ШаблонДереваОтборПоСвойствамПланаОбмена;  // шаблон дерева значений для правил регистрации по свойствам Плана обмена
...
Функция ПолучитьИнформациюОПравилах() Экспорт
	...
	СтрокаИнформации = НСтр("ru = 'Правила регистрации объектов
						|Дата создания:   %1
						|План обмена:     %2
						|Конфигурация:    %3'");
	//
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаИнформации,
					Регистрация.ДатаВремяСоздания,
					Регистрация.ИмяПланаОбмена,
...
Процедура ЗагрузитьЭлементОтбораПланаОбмена(Правила, НоваяСтрока)
	...
	Пока Правила.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "СвойствоПланаОбмена" Тогда
			...
			// Название ТЧ записано в кватратных скобках "[...]"
			...
			ПозСкобки1 = Найти(ПолноеНаименованиеСвойства, "[");
			
			Если ПозСкобки1 <> 0 Тогда
				
				ПозСкобки2 = Найти(ПолноеНаименованиеСвойства, "]");
...
Функция ИнициализацияСообщений()
	...
	Сообщения.Вставить(5, НСтр("ru = 'Загружаемые правила регистрации не предназначены для текущего плана обмена.'"));
