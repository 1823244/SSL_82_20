	...
	Пока УдаляемыеОбъекты.Количество() > 0 Цикл
		...
		Для Каждого СтрокаТаблицы Из НайденныеДанные Цикл
			...
			Если ВедущиеИзмерения.Количество() > 0 Тогда
				УдаляемыйОбъектТолькоВВедущихИзмерениях = Истина;
				...
				Если НаборЗаписей.Количество() > 0 Тогда
					...
					Запись = ТаблицаЗаписи[0];
					Для каждого Колонка Из ТаблицаЗаписи.Колонки Цикл
						
						Если Запись[Колонка.Имя] = СтрокаТаблицы.Ссылка
						   И ВедущиеИзмерения[Колонка.Имя] = Неопределено Тогда
							
							УдаляемыйОбъектТолькоВВедущихИзмерениях = Ложь;
							Прервать;
						КонецЕсли;
					КонецЦикла;
				КонецЕсли;
				Если УдаляемыйОбъектТолькоВВедущихИзмерениях Тогда
					Продолжить;
				КонецЕсли;
			КонецЕсли;
