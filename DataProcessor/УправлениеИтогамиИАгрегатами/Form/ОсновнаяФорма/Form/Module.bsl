...
Процедура УстановитьПериодИтогов(Команда)
	...
	Результат = ОткрытьФормуМодально("Обработка.УправлениеИтогамиИАгрегатами.Форма.ФормаВыбораПериода", ПараметрыФормы);
	...
	ПараметрыИтогов.Вставить("Значение1",              Результат.ПериодДляРегНакоп);
	ПараметрыИтогов.Вставить("Значение2",              Результат.ПериодДляРегБухг);
...
Процедура ПерестроитьАгрегаты(Команда)
	...
	ПараметрыИтогов.Вставить("Значение1",              Результат.ОтносительныйРазмер);
	ПараметрыИтогов.Вставить("Значение2",              Результат.МинимальныйЭффект);
...
Процедура ОчиститьАгрегатыПоРегистрам(Команда)
	...
	ТекстВопроса = НСтр("ru='Очистка агрегатов приведет к существенному замедлению отчетов.
	                        |Выполнить очистку?'");
...
Процедура АгрегатыОптимальные(Команда)
	...
	ПараметрыИтогов = Новый Структура;
	ПараметрыИтогов.Вставить("ОбрабатыватьЗаголовок",  НСтр("ru='Получение списка оптимальных агрегатов ...'"));
	ПараметрыИтогов.Вставить("ПослеПроцесса",          НСтр("ru='Получение списка оптимальных агрегатов завершено'"));
	ПараметрыИтогов.Вставить("Действие",               "ПолучитьОптимальныеАгрегаты");
	ПараметрыИтогов.Вставить("МассивСтрок",            Элементы.АгрегатыПоРегистрам.ВыделенныеСтроки);
	ПараметрыИтогов.Вставить("Поле",                   "Наименование");
	ПараметрыИтогов.Вставить("Значение1",              Результат.ОтносительныйРазмер);
	ПараметрыИтогов.Вставить("Значение2",              Неопределено);
	ПараметрыИтогов.Вставить("ТекстСообщенияОбОшибке", НСтр("ru='Ошибка получения списка оптимальных агрегатов.'"));
...
Процедура ЗаполнитьАгрегатыИВыполнитьПерестроение(Команда)
	...
	Если МассивДействий.Количество() = 0 Тогда
		Предупреждение(НСтр("ru='Отсутствуют регистры, для которых можно выполнить выбранное действие.'"));
		Возврат;
	КонецЕсли;
			...
			Для Индекс1=0 По ОптимальныеАгрегаты.Агрегаты.ВГраница() Цикл
				...
				Для Индекс2=0 По МетаданныеАгрегаты.Количество() - 1 Цикл
					...
					Если ОптимальныеАгрегаты.Агрегаты[Индекс1].Периодичность
					   ...
					   И НайденыеАгрегаты[Индекс2] = Неопределено
...
&НаКлиенте
Функция ПолучитьПараметрыПерестроения(Знач РежимПерестроения)
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ОтносительныйРазмер", ?(РежимПерестроения, ОтносительныйРазмер, ОптимальныйОтносительныйРазмер));
	ПараметрыФормы.Вставить("МинимальныйЭффект",   ?(РежимПерестроения, МинимальныйЭффект, 0));
	ПараметрыФормы.Вставить("РежимПерестроения",   РежимПерестроения);
	
	Результат = ОткрытьФормуМодально("Обработка.УправлениеИтогамиИАгрегатами.Форма.ФормаПараметровПерестроения", ПараметрыФормы);
	
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		Если РежимПерестроения Тогда
			ОтносительныйРазмер = Результат.ОтносительныйРазмер;
			МинимальныйЭффект   = Результат.МинимальныйЭффект;
		Иначе
			ОптимальныйОтносительныйРазмер = Результат.ОтносительныйРазмер;
		КонецЕсли;
