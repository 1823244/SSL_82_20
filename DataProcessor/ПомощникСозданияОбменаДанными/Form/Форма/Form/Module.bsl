...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Пользователи.РолиДоступны("ДобавлениеИзменениеОбменовДанными") Тогда
		ОбменДаннымиСервер.СообщитьОбОшибке(НСтр("ru = 'Нет прав на добавление/изменение обменов данными.'"), Отказ);
		Возврат;
	КонецЕсли;
	...
	Если Не Параметры.Свойство("ИмяПланаОбмена", Объект.ИмяПланаОбмена) Тогда
		...
		НСтр("ru = 'Работа помощника поддерживается только при вызове из командного интерфейса. Работа помощника завершена.'"), Отказ);
	...
	Заголовок = Метаданные.ПланыОбмена[Объект.ИмяПланаОбмена].Синоним + " " + НСтр("ru='(создание)'");
	
	ИмяФайлаНастроекДляПриемника = МенеджерПланаОбмена.ИмяФайлаНастроекДляПриемника() + ".xml";
	
	НастройкаОтборовНаУзле    = ОбменДаннымиСервер.ЗначениеПоТипу(МенеджерПланаОбмена.НастройкаОтборовНаУзле(), "Структура");
	ЗначенияПоУмолчаниюНаУзле = ОбменДаннымиСервер.ЗначениеПоТипу(МенеджерПланаОбмена.ЗначенияПоУмолчаниюНаУзле(), "Структура");
	...
	Если ИспользоватьТранспортСообщенийОбменаCOM
		ИЛИ ИспользоватьТранспортСообщенийОбменаWS Тогда
		
		НастройкаОтборовНаУзлеБазыКорреспондента    = ОбменДаннымиСервер.ЗначениеПоТипу(МенеджерПланаОбмена.НастройкаОтборовНаУзлеБазыКорреспондента(), "Структура");
		ЗначенияПоУмолчаниюНаУзлеБазыКорреспондента = ОбменДаннымиСервер.ЗначениеПоТипу(МенеджерПланаОбмена.ЗначенияПоУмолчаниюНаУзлеБазыКорреспондента(), "Структура");
		...
		ОписаниеОграниченийПередачиДанныхБазыКорреспондента = МенеджерПланаОбмена.ОписаниеОграниченийПередачиДанныхБазыКорреспондента(НастройкаОтборовНаУзлеБазыКорреспондента);
		ОписаниеЗначенийПоУмолчаниюБазыКорреспондента       = МенеджерПланаОбмена.ОписаниеЗначенийПоУмолчаниюБазыКорреспондента(ЗначенияПоУмолчаниюНаУзлеБазыКорреспондента);
		
		НастройкиПараметровУчетаКорреспондентаПоясняющаяНадпись = МенеджерПланаОбмена.ПояснениеДляНастройкиПараметровУчетаБазыКорреспондента();
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если ДлительнаяОперация Тогда
		Предупреждение(НСтр("ru = 'Выполняется создание обмена данными.
...
Процедура КомандаГотово(Команда)
	...
	ОбменДаннымиКлиентПереопределяемый.ПриЗавершенииРаботыПомощникаСозданияОбмена(ЭтаФорма, ЗапуститьМенеджерЗаданий());
...
Процедура ПроверитьПодключениеCOM(Команда)
	...
	ОбменДаннымиСервер.ВыполнитьПроверкуУстановкиВнешнегоСоединения(Отказ, СтруктураНастроек, ОшибкаПодключенияКомпоненты);
...
Процедура КакОпределитьПараметрыПодключенияКВебСервису(Команда)
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru = 'Как определить параметры подключения ко второй информационной базе'"));
	
	ОткрытьФормуМодально("Обработка.ПомощникСозданияОбменаДанными.Форма.ДополнительноеОписание", ПараметрыФормы, ЭтаФорма);
...
Процедура КакСформироватьФайлСНастройкамиОбменаДанными(Команда)
	...
	ПараметрыФормы.Вставить("Заголовок", НСтр("ru = 'Как сформировать файл с настройками обмена данными'"));
...
Процедура УзнатьПрефиксЭтойИнформационнойБазы(Команда)
	...
	ОткрытьФормуМодально("Обработка.ПомощникСозданияОбменаДанными.Форма.УзнатьПрефиксЭтойИнформационнойБазы", , ЭтаФорма);
	
КонецПроцедуры

&НаКлиенте
Процедура ВыполнитьЗагрузкуДанныхВоВторойИнформационнойБазе(Команда)
	
	ОбменДаннымиКлиент.ЗапуститьПриложение1СПредприятиеИВыполнитьИнтерактивнуюЗагрузкуДанных(СтруктураПараметровПодключенияВнешнегоСоединения(), Объект.ИмяПланаОбмена);
...
Процедура ОбработчикОжиданияДлительнойОперации()
	...
	Иначе // Failed, Canceled
		...
		ТекстВопроса = НСтр("ru = 'При создании обмена данными возникли ошибки.
...
Процедура ПолучитьОписаниеОграниченийПередачиДанныхБазыКорреспондента(НастройкаОтборовНаУзлеБазыКорреспондента)
	...
	ОписаниеОграниченийПередачиДанныхБазыКорреспондента = ПланыОбмена[Объект.ИмяПланаОбмена].ОписаниеОграниченийПередачиДанныхБазыКорреспондента(НастройкаОтборовНаУзлеБазыКорреспондента);
...
Процедура ПолучитьОписаниеЗначенийПоУмолчаниюБазыКорреспондента(ЗначенияПоУмолчаниюНаУзлеБазыКорреспондента)
	...
	ОписаниеЗначенийПоУмолчаниюБазыКорреспондента = ПланыОбмена[Объект.ИмяПланаОбмена].ОписаниеЗначенийПоУмолчаниюБазыКорреспондента(ЗначенияПоУмолчаниюНаУзлеБазыКорреспондента);
...
Процедура ИнициализацияОбменаДаннымиНаКлиенте(Отказ)
	...
	Если Отказ Тогда
		...
		Предупреждение(НСтр("ru = 'Во время выгрузки данных возникли ошибки(см. журнал регистрации).'"));
...
Процедура ПроверитьПодключение(ВидТранспорта)
	...
	СтрокаСообщения = ?(Отказ,  НСтр("ru = 'При установке подключения возникли ошибки(см. Журнал регистрации).'"),
...
Процедура ПроверитьПодключениеWSНаСервере(Отказ, РасширеннаяПроверка, ПредлагатьПерейтиВЖурналРегистрации)
	...
	Если Объект.ВерсияКорреспондента_2_0_1_6 Тогда
	...
	Если РасширеннаяПроверка Тогда
		...
		Если Не ПараметрыПриемника.ПланОбменаСуществует Тогда
			...
			Сообщение = НСтр("ru = 'Вторая информационная база не предназначена для обмена с текущей информационной базой.'");
...
Процедура ОтображениеЭлементовКомпозицииКартинокПервойСтраницыМастера()
	...
	ЭтоПерваяИнформационнаяБаза = (ВариантРаботыМастера = "НастроитьНовыйОбменДанными");
...
Процедура ВыбратьФайлНастроекОбмена(Интерактивно)
	...
	Если ПоместитьФайл(АдресВременногоХранилища, ИмяФайлаПоУмолчанию, ВыбранноеИмяФайла, Интерактивно, УникальныйИдентификатор) Тогда
		...
		Если Отказ Тогда
			Предупреждение(НСтр("ru = 'Указан неправильный файл настроек обмена данными. Укажите корректный файл.'"));
			Возврат;
		КонецЕсли;
...
Процедура ОбновитьИнформациюСопоставленияПоСтрокеНаСервере(Отказ, КлючиСтрок)
	...
	Если Не Отказ Тогда
		...
		ЗаполнитьЗначенияСвойств(Объект, ПомощникИнтерактивногоОбменаДанными, "НомерВходящегоСообщения");
...
Функция Подключаемый_СтраницаМастераНастройкаПараметров_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(Объект.НаименованиеЭтойБазы) Тогда
		...
		НСтрока = НСтр("ru = 'Укажите наименование этой информационной базы.'");
	...
	Если ПустаяСтрока(Объект.НаименованиеВторойБазы) Тогда
		...
		НСтрока = НСтр("ru = 'Укажите наименование второй информационной базы.'");
...
Функция Подключаемый_СтраницаМастераНастройкаПараметровОбменаДанными_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)
	...
	Если ЭтоПереходДалее Тогда
		...
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ПолучениеОписанияКонтекста");
		ПараметрыФормы.Вставить("Настройки", КонтекстФормыНастройкиУзлов);
...
Функция Подключаемый_СтраницаМастераНастройкаПараметров_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)
	...
	Если НЕ    (Объект.ИспользоватьПараметрыТранспортаEMAIL
		...
		НСтрока = НСтр("ru = 'Не указаны настройки ни для одного вида обмена данными.
...
Функция Подключаемый_СтраницаМастераИтогНастройкиОбмена_ПриОткрытии_ВнешнееСоединение(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)
	...
	Иначе
		...
		СтрокаСообщения = НСтр("ru = '%1
		|Настройки обмена данными для этой информационной базы:
		|========================================================
		|%2%3Префикс информационной базы: %4
		|
		|Настройки обмена данными для второй информационной базы:
		|========================================================
...
Функция Подключаемый_СтраницаМастераОжиданиеАнализаДанныхСозданиеНастройкиОбмена_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	Если Отказ Тогда
		...
		Предупреждение(НСтр("ru = 'Возникли ошибки на этапе создания настройки обмена данными.
...
Процедура СтраницаМастераОжиданиеАнализаДанныхАвтоматическоеСопоставление_ОбработкаДлительнойОперации(Отказ)
	...
	ПомощникИнтерактивногоОбменаДанными.ВыполнитьАнализСообщенияОбмена(Отказ, Ложь);
...
Функция Подключаемый_СтраницаМастераОжиданиеСинхронизацииСправочниковЗагрузка_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ОбменДаннымиСервер.ВыполнитьЗагрузкуДляУзлаИнформационнойБазыЧерезФайл(Отказ, Объект.УзелИнформационнойБазы, Объект.ИмяФайлаСообщенияОбмена);
...
Процедура ПроверитьНастройкиПараметровУчетаНаСервере(
	...
	КодУзла = ОбщегоНазначения.ПолучитьЗначениеРеквизита(Объект.УзелИнформационнойБазы, "Код");
	...
	Если ТипСоединения = "ВебСервис" Тогда
		...
		КодУзла = ОбменДаннымиПовтИсп.ПолучитьКодЭтогоУзлаДляПланаОбмена(Объект.ИмяПланаОбмена);
...
Процедура СтраницаМастераНастройкаПараметров_ПриПереходеДалееНаСервере(Отказ)
	...
	Если Не ПланыОбмена[Объект.ИмяПланаОбмена].НайтиПоКоду(ОбменДаннымиСервер.КодУзлаПланаОбменаСтрокой(Объект.ПрефиксИнформационнойБазыПриемника)).Пустая() Тогда
		...
		НСтрока = НСтр("ru = 'Значение префикса второй информационной базы не уникально.
			|В системе уже существует обмен данными для информационной базы с указанным префиксом.
			|Измените значение префикса или используйте существующий обмен.'");
		//
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтрока,, "Объект.ПрефиксИнформационнойБазыПриемника",, Отказ);
...
Процедура ПроверитьНаличиеСуществующегоОбменаДаннымиСВторойБазой(Отказ)
	...
	Если Не ПустаяСтрока(КодУзла)
		...
		Сообщение = НСтр("ru = 'Обмен данными уже настроен в системе. Используйте существующий обмен.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Сообщение,,,, Отказ);
...
Процедура ТаблицаПереходовВторогоЭтапаНастройкиОбмена()
	...
	ТаблицаПереходовНоваяСтрока(6, "СтраницаМастераИтогНастройкиОбмена",                      "", "СтраницаНавигацииОкончание",,,"СтраницаМастераИтогНастройкиОбмена_ПриОткрытии");
...
Процедура ТаблицаПереходовНастройкиОбменаЧерезВнешнееСоединение()
	...
	ТаблицаПереходовНоваяСтрока(4, "СтраницаМастераНастройкаПараметровДляВнешнегоСоединенияПервойБазы", "", "СтраницаНавигацииПродолжение", "СтраницаМастераНастройкаПараметровДляВнешнегоСоединенияПервойБазы_ПриПереходеДалее");
	ТаблицаПереходовНоваяСтрока(5, "СтраницаМастераНастройкаПараметровДляВнешнегоСоединенияВторойБазы", "", "СтраницаНавигацииПродолжение", "СтраницаМастераНастройкаПараметровДляВнешнегоСоединенияВторойБазы_ПриПереходеДалее");
...
Процедура ТаблицаПереходовНастройкиОбменаЧерезВебСервис()
	...
	ТаблицаПереходовНоваяСтрока(12, "СтраницаМастераОжиданиеСинхронизацииСправочниковЗагрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииСправочниковЗагрузка_ОбработкаДлительнойОперации");
	ТаблицаПереходовНоваяСтрока(13, "СтраницаМастераОжиданиеСинхронизацииСправочниковВыгрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииСправочниковВыгрузка_ОбработкаДлительнойОперации");
	ТаблицаПереходовНоваяСтрока(14, "СтраницаМастераОжиданиеСинхронизацииСправочниковВыгрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииСправочниковВыгрузкаДлительнаяОперация_ОбработкаДлительнойОперации");
	ТаблицаПереходовНоваяСтрока(15, "СтраницаМастераОжиданиеСинхронизацииСправочниковВыгрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииСправочниковВыгрузкаДлительнаяОперацияОкончание_ОбработкаДлительнойОперации");
	...
	ТаблицаПереходовНоваяСтрока(19, "СтраницаМастераОжиданиеСинхронизацииДанныхЗагрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииДанныхЗагрузка_ОбработкаДлительнойОперации");
	ТаблицаПереходовНоваяСтрока(20, "СтраницаМастераОжиданиеСинхронизацииДанныхЗагрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииДанныхЗагрузкаДлительнаяОперация_ОбработкаДлительнойОперации");
	ТаблицаПереходовНоваяСтрока(21, "СтраницаМастераОжиданиеСинхронизацииДанныхЗагрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииДанныхЗагрузкаДлительнаяОперацияОкончание_ОбработкаДлительнойОперации");
	ТаблицаПереходовНоваяСтрока(22, "СтраницаМастераОжиданиеСинхронизацииДанныхВыгрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииДанныхВыгрузка_ОбработкаДлительнойОперации");
	ТаблицаПереходовНоваяСтрока(23, "СтраницаМастераОжиданиеСинхронизацииДанныхВыгрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииДанныхВыгрузкаДлительнаяОперация_ОбработкаДлительнойОперации");
	ТаблицаПереходовНоваяСтрока(24, "СтраницаМастераОжиданиеСинхронизацииДанныхВыгрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииДанныхВыгрузкаДлительнаяОперацияОкончание_ОбработкаДлительнойОперации");
...
Процедура ТаблицаПереходовРасширеннойНастройкиОбменаССервисом()
	...
	// Синхронизация справочников
	ТаблицаПереходовНоваяСтрока(11, "СтраницаМастераОжиданиеСинхронизацииСправочниковЗагрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииСправочниковЗагрузка_ОбработкаДлительнойОперации");
	ТаблицаПереходовНоваяСтрока(12, "СтраницаМастераОжиданиеСинхронизацииСправочниковВыгрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииСправочниковВыгрузка_ОбработкаДлительнойОперации");
	ТаблицаПереходовНоваяСтрока(13, "СтраницаМастераОжиданиеСинхронизацииСправочниковВыгрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииСправочниковВыгрузкаДлительнаяОперация_ОбработкаДлительнойОперации");
	ТаблицаПереходовНоваяСтрока(14, "СтраницаМастераОжиданиеСинхронизацииСправочниковВыгрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииСправочниковВыгрузкаДлительнаяОперацияОкончание_ОбработкаДлительнойОперации");
	...
	// Синхронизация всех данных, кроме справочников
	ТаблицаПереходовНоваяСтрока(18, "СтраницаМастераОжиданиеСинхронизацииДанныхЗагрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииДанныхЗагрузка_ОбработкаДлительнойОперации");
	ТаблицаПереходовНоваяСтрока(19, "СтраницаМастераОжиданиеСинхронизацииДанныхЗагрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииДанныхЗагрузкаДлительнаяОперация_ОбработкаДлительнойОперации");
	ТаблицаПереходовНоваяСтрока(20, "СтраницаМастераОжиданиеСинхронизацииДанныхЗагрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииДанныхЗагрузкаДлительнаяОперацияОкончание_ОбработкаДлительнойОперации");
	ТаблицаПереходовНоваяСтрока(21, "СтраницаМастераОжиданиеСинхронизацииДанныхВыгрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииДанныхВыгрузка_ОбработкаДлительнойОперации");
	ТаблицаПереходовНоваяСтрока(22, "СтраницаМастераОжиданиеСинхронизацииДанныхВыгрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииДанныхВыгрузкаДлительнаяОперация_ОбработкаДлительнойОперации");
	ТаблицаПереходовНоваяСтрока(23, "СтраницаМастераОжиданиеСинхронизацииДанныхВыгрузка",, "СтраницаНавигацииОжидание",,,, Истина, "СтраницаМастераОжиданиеСинхронизацииДанныхВыгрузкаДлительнаяОперацияОкончание_ОбработкаДлительнойОперации");
