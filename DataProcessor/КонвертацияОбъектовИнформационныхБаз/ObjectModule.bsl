...
// Функция-свойство: результат выполнения обмена данными
//
// Тип: Строка - Строковое представление значения перечисления ПеречислениеСсылка.РезультатыВыполненияОбмена
//
...
Функция КлючСообщенияЖурналаРегистрации()
	...
	Если ТипЗнч(КлючСообщенияЖурналаРегистрации) <> Тип("Строка")
		...
		КлючСообщенияЖурналаРегистрации = НСтр("ru = 'Обмен данными'");
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ ОБРАБОТКИ СОБЫТИЙ ВНЕШНЕГО СОЕДИНЕНИЯ
...
Процедура ЗаписатьОбъектВИБ(Объект, Тип, ЗаписатьОбъект = Ложь)
...
Процедура УстановитьСвойстваКоллекцииФайлаОбмена(Объект, КоллекцияФайлаОбмена, ИнформацияОТипах, ИмяВетки, ПараметрыОбъекта, НомерЗаписи, Знач ИмяТабличнойЧасти, Знач ИмяПоляСортировки)
...
Процедура ЗагрузитьТабличнуюЧасть(Объект, ИмяТабличнойЧасти, ОбщаяИнформацияОТипеДокумента, ПараметрыОбъекта, ПКО)
	...
	Если Не Результат Тогда
		...
		КлючевыеПоляПоиска = СтроковыеФункцииКлиентСервер.ПолучитьСтрокуИзМассиваПодстрок(МассивКлючевыхПолейПоиска);
...
Процедура ЗаполнитьКоллекциюФайлаОбмена(Объект, КоллекцияФайлаОбмена, ИмяТабличнойЧасти, ОбщаяИнформацияОТипеДокумента, ПараметрыОбъекта, МассивКлючевыхПолейПоиска, ИмяПоляСортировки)
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "Запись" Тогда
			...
			УстановитьСвойстваКоллекцииФайлаОбмена(Объект, КоллекцияФайлаОбмена, ИнформацияОТипах, ИмяВетки, ПараметрыОбъекта, НомерЗаписи, ИмяТабличнойЧасти, ИмяПоляСортировки);
...
Функция ПроверитьНаличиеКоллизийДляОбъекта(Объект, ИмяТипаОбъекта, ДополнительнаяИнформация = "", ПриоритетОбъектаОбмена)
	...
	Если ЕстьКоллизия Тогда
		...
		Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Объект)) Тогда
			...
			ЗП.Вставить("Данные", ОбщегоНазначения.РасширенноеПредставлениеОбъекта(Объект.Ссылка));
			...
			Если ИмяТипаОбъекта = "Документ" Тогда
				...
				Если РежимЗагрузкиДанныхВИнформационнуюБазу() Тогда
					...
					Попытка
						...
						// записываем документ, информацию о необходимости его провести или отменить проведение отделько фиксируем
						...
						Если ЗаписатьОбъект Тогда
							...
							Если РежимЗаписи = РежимЗаписиДокумента.Запись Тогда
								...
								ЗаписатьОбъектВИБ(Объект, ТипОбъекта, ЗаписатьОбъект);
...
Функция ПолучитьСтруктуруРеквизитовРегистрации(ТаблицаПКС)
	...
	Для Каждого ПКС ИЗ МассивСтрокПКС Цикл
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Обмен данными.Загрузка правил конвертации'"),
...
Процедура ПрочитатьДанныеПоОбмену()
	...
	Если УзелОбменаЗагрузкаДанных.НомерПринятого >= НомерСообщения() Тогда
	...
	Если УдалитьРегистрациюИзменений Тогда
		...
		РегистрыСведений.ИзмененияОбщихДанныхУзлов.УдалитьРегистрациюИзменений(УзелОбменаЗагрузкаДанных, НомерПринятогоСообщения());
		
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияФорматаВходящегоСообщенияОбмена(), "3.1.0.0") >= 0 Тогда
			
			РегистрыСведений.ОбщиеНастройкиУзловИнформационныхБаз.ЗафиксироватьВыполнениеКорректировкиИнформацииСопоставления(УзелОбменаЗагрузкаДанных, НомерПринятогоСообщения());
			
		КонецЕсли;
		
		Если ОбменДаннымиСервер.УстановленПризнакНачальнойВыгрузкиДанных(УзелОбменаЗагрузкаДанных) Тогда
			
			РегистрыСведений.ОбщиеНастройкиУзловИнформационныхБаз.СнятьПризнакНачальнойВыгрузкиДанных(УзелОбменаЗагрузкаДанных, НомерПринятогоСообщения());
...
Процедура ПрочитатьОбщиеДанныеУзлов()
	...
	Если ОбменДаннымиСобытия.ДанныеРазличаются(ОбщийУзел, ОбщийУзел.Ссылка.ПолучитьОбъект()) Тогда
		...
		Попытка
			...
			ОбщийУзел.ДополнительныеСвойства.Вставить("ПолучениеСообщенияОбмена");
...
Процедура ВыгрузитьОбщиеДанныеУзлов(Знач НомерОтправленного)
	...
	СвойстваОбщегоУзла = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ОбщиеДанныеУзлов);
	...
	Для Каждого Свойство Из СвойстваОбщегоУзла Цикл
		...
		Если ТабличныеЧастиОбщегоУзла.Найти(Свойство) <> Неопределено Тогда
			...
			Для Каждого Реквизит Из МетаданныеОбщегоУзла.ТабличныеЧасти[Свойство].Реквизиты Цикл
				...
				ПКС = ПКС.ПравилаГруппы.Добавить();
				ПКС.ЭтоГруппа = Ложь;
				ПКС.ВидИсточника = "Реквизит";
				ПКС.ВидПриемника = "Реквизит";
				ПКС.Источник = Реквизит.Имя;
				ПКС.Приемник = Реквизит.Имя;
...
Процедура ВыполнитьВыгрузку(СтрокаСообщенияОбОшибке = "")
	...
	НомерОтправленного = ОбщегоНазначения.ПолучитьЗначениеРеквизита(УзелДляОбмена, "НомерОтправленного") + ?(ВыгрузитьИнформациюСопоставления, 2, 1);
...
Процедура ВыполнитьВыгрузкуРегистраСопоставленияОбъектов(ЗаписьСообщения, СтрокаСообщенияОбОшибке)
	...
	ВыборкаИзменений = ПланыОбмена.ВыбратьИзменения(ЗаписьСообщения.Получатель, ЗаписьСообщения.НомерСообщения, Метаданные.РегистрыСведений.СоответствияОбъектовИнформационныхБаз);
...
Процедура ВыполнитьВыгрузкуЗарегистрированныхДанных(ЗаписьСообщения, СтрокаСообщенияОбОшибке, ТаблицаПравилВыгрузкиИспользуемые)
	...
	Пока ВыборкаИзменений.Следующий() Цикл
		...
		Если ТипДанныхДляВыгрузки <> ТипРегистрСоответствия Тогда
			...
			ОбменДаннымиСобытия.ПриОтправкеДанных(Данные, ОтправкаЭлемента, УзелДляОбмена, ИмяПланаОбмена(), НачальнаяВыгрузкаДанных);
