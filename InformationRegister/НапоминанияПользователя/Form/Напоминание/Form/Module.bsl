...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ИначеЕсли ТекущийОбъект.СпособУстановкиВремениНапоминания = Перечисления.СпособыУстановкиВремениНапоминания.ОтносительноВремениПредмета Тогда
		ДатаВИсточнике = НапоминанияПользователяСервер.ПолучитьЗначениеРеквизитаПредмета(Объект.Источник, Объект.ИмяРеквизитаИсточника);
		...
		Иначе
			...
			Сообщение.Текст = НСтр("ru = 'Время в реквизите предмета не определено'");
			Сообщение.Поле = "ИмяРеквизитаИсточника";
	...
	ИначеЕсли ТекущийОбъект.СпособУстановкиВремениНапоминания = Перечисления.СпособыУстановкиВремениНапоминания.Периодически Тогда
		БлижайшееВремяНапоминания = НапоминанияПользователяСервер.ПолучитьБлижайшуюДатуСобытияПоРасписанию(Расписание);
...
Процедура СпособУстановкиВремениНапоминанияПриИзменении(Элемент)
	...
	Иначе
		Если Элементы.СпособУстановкиВремениНапоминания.СписокВыбора.НайтиПоЗначению(СпособУстановкиВремениНапоминания) = Неопределено Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru = 'Интервал времени не определён.'");
			Сообщение.Поле = "СпособУстановкиВремениНапоминания";
			Сообщение.УстановитьДанные(Объект);
			Сообщение.Сообщить();			
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриИзмененииИнтервалаВремени(Элемент)
	...
	Иначе
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Интервал времени не определён'");
		Сообщение.Поле = "ИнтервалВремениСтрокой";
		Сообщение.УстановитьДанные(Объект);
		Сообщение.Сообщить();
	КонецЕсли;
КонецПроцедуры
...
Процедура Прекратить(Команда)
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		...
		Если ИсходныеПараметры <> Неопределено Тогда 
			НапоминанияПользователяВызовСервера.ОтключитьНапоминание(ИсходныеПараметры, Ложь);
			НапоминанияПользователяКлиент.УдалитьЗаписьИзКэшаОповещений(ИсходныеПараметры);
			Оповестить("Запись_НапоминанияПользователя", Новый Структура, Объект.ИсходныйКлючЗаписи);
			ОповеститьОбИзменении(Объект.ИсходныйКлючЗаписи);
		КонецЕсли;
...
Процедура ЗаполнитьСписокРеквизитовИсточника()
	...
	// получаем переопределённый массив реквизитов
	СтандартныеПодсистемыПереопределяемый.ПриЗаполненииСпискаРеквизитовИсточникаСДатамиДляНапоминания(Объект.Источник, МассивРеквизитов);
...
Процедура ЗаполнитьВариантыПериодичности()
	...
	СписокРасписаний = НапоминанияПользователяКлиентСервер.ПолучитьСтандартныеРасписанияДляНапоминания();
...
Процедура ОткрытьДиалогНастройкиРасписания()
	...
	Если ДиалогРасписания.ОткрытьМодально() Тогда
		...
		Если Не РасписаниеСоответствуетТребованиям(Расписание) Тогда 
			Предупреждение(НСтр("ru = 'Периодичность в течение дня не поддерживается, соответствующие настройки очищены.'"));
		КонецЕсли;
...
Процедура ОпределитьВыбранныйВариантПериодичности()
	...
	СтандартныеРасписания = НапоминанияПользователяКлиентСервер.ПолучитьСтандартныеРасписанияДляНапоминания();
