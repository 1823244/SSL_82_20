...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ЗапроситьПодтверждениеЗакрытияФормы(
		Отказ, 
		ЕстьРедактируемые, , 
		НСтр("ru = 'В списке остались макеты отмеченные как редактируемые. Закрыть форму?'")); 
	
КонецПроцедуры

////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЦЫ ФОРМЫ СписокМакетов
			...
			Исключение
				...
				СообщениеОбОшибке = СообщениеОбОшибке + Символы.ПС + 
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Файл %1 открыт другим приложением. Действие не выполнено.'"),
...
#Если ВебКлиент Тогда
	Попытка
		Если НЕ ПоместитьФайлы(ПомещаемыеФайлы, ПомещенныеФайлы, , Ложь) Тогда
			Предупреждение(НСтр("ru = 'Не удалось поместить файл макета в информационную базу.'"));
			Возврат;
		КонецЕсли;
	Исключение
		СообщениеОбОшибке = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка при помещении файла макета в информационную базу: %1. Убедитесь, что приложение работы с файлом закрыто.'"), СообщениеОбОшибке));
		Возврат;
	КонецПопытки;
#КонецЕсли
