...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ЗапроситьПодтверждениеЗакрытияФормы(Отказ, Модифицированность, ВыполненаКомандаЗакрытия); 
	
КонецПроцедуры

////////////////////////////////////////////////////////////////////////////////
// ОБРАБОТЧИКИ СОБЫТИЙ ТАБЛИЦЫ ФОРМЫ Таблица контактов

&НаКлиенте
Процедура ТаблицаКонтактовКонтактНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
...
&НаКлиенте
Процедура ТаблицаКонтактовТекущийАдресКонтактаНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
...
Процедура ТаблицаКонтактовТекущийАдресКонтактаОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") И 
		(ВыбранноеЗначение.Адрес <> ТекущиеДанные.ТекущийАдресКонтакта ИЛИ
			ВыбранноеЗначение.Вид <> ТекущиеДанные.ТекущийВидКонтактнойИнформации) Тогда
...
Процедура ТаблицаКонтактовКонтактНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ВзаимодействияКлиент.ВыбратьКонтакт(
			...
			ТекущиеДанные.Контакт, Истина, Ложь, Ложь,Истина
		) Тогда
		ЗаполнитьАдресаКонтакта(Элементы.ТаблицаКонтактов.ТекущаяСтрока);
		УстановитьСнятьПризнакИзменятьЕслиНеобходимо(ТекущиеДанные);
		Модифицированность = Истина;
...
Процедура КомандаОКВыполнить()
	...
	Для каждого СтрокаТаблицыКонтактов Из ТаблицаКонтактов Цикл
		...
		СтруктураДанные = Новый Структура("Представление,Адрес,Контакт,Группа",
		                                  СтрокаТаблицыКонтактов.Представление,
		                                  СтрокаТаблицыКонтактов.Адрес,
		                                  СтрокаТаблицыКонтактов.Контакт,
		                                  СтрокаТаблицыКонтактов.Группа);
		
		Результат.Добавить(СтруктураДанные);
...
Процедура ИзменитьКонтактнуюИнформациюДляВыбранных()
	...
	Выборка = Запрос.Выполнить().Выбрать();
		...
		Позиция = Найти(СтрокаТабличнойЧасти.АдресЭП, "@");
		...
		Попытка
			...
			Для каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				Если НайденнаяСтрока.Изменять Тогда
					...
					ИначеЕсли НайденнаяСтрока.СписокНайденныхКонтактов.Количество() = 1 И
						      НайденнаяСтрока.СписокНайденныхКонтактов.Получить(0) <> КонтактОбъект.Ссылка Тогда
		...
		Исключение
			...
			|%2'"), Выборка.Контакт,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура УстановитьСнятьПризнакИзменятьЕслиНеобходимо(ТекущиеДанные)
	...
	Если (НЕ ЗначениеЗаполнено(ТекущиеДанные.Адрес) ИЛИ Не ЗначениеЗаполнено(ТекущиеДанные.Контакт) ИЛИ
		 ВРег(ТекущиеДанные.Адрес) = ВРег(ТекущиеДанные.ТекущийАдресКонтакта)) Тогда
