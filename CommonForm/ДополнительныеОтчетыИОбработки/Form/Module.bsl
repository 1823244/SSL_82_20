...
Процедура ОтменитьВыполнениеОбработки(Команда)
	ДлительныеОперацииКлиент.ОтменитьВыполнениеЗадания(ФоновоеЗаданиеИдентификатор);
КонецПроцедуры
...
Процедура ВыполнитьОткрытиеФормыОбработки()
	ИмяОбработки = ДополнительныеОтчетыИОбработки.ПодключитьВнешнююОбработку(ВыполняемаяОбработка.Ссылка);
