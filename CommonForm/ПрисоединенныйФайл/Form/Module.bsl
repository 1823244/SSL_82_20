...
Процедура ОбновитьИзФайлаНаДиске(Команда)
	...
	Если ПрисоединенныеФайлыКлиент.ВыбратьФайлНаДискеИПоместитьВХранилище(
...
Процедура ПодписатьФайлЭЦП(Команда)
	...
	Если ПрисоединенныеФайлыКлиент.ВыбратьСертификатыЭЦПИСформироватьДанныеПодписи(
...
#КонецЕсли
	...
	Если ПодключитьРасширениеРаботыСФайлами() Тогда
		...
		МассивПодписей = ПрисоединенныеФайлыКлиент.ПолучитьМассивПодписей(
			ЭтаФорма.Объект.Ссылка, УникальныйИдентификатор);
		
		Если МассивПодписей.Количество() > 0 Тогда
			ЗанестиИнформациюОПодписях(МассивПодписей);
			
			ПрисоединенныеФайлыКлиент.ОповеститьОДобавленииПодписиИзФайла(
...
Процедура Зашифровать(Команда)
	...
	Если НЕ ПрисоединенныеФайлыКлиент.ПолучитьЗашифрованныеДанные(
			ЭтаФорма.Объект.Ссылка,
			ДанныеФайла,
			УникальныйИдентификатор,
			ЗашифрованныеДанные,
	...
	ПрисоединенныеФайлыКлиент.ОповеститьОбИзмененииИУдалитьФайлВРабочемКаталоге(
...
Процедура Расшифровать(Команда)
	...
	Если НЕ ПрисоединенныеФайлыКлиент.ПолучитьРасшифрованныеДанные(
			ЭтаФорма.Объект.Ссылка,
			ДанныеФайла,
	...
	ПрисоединенныеФайлыКлиент.ОповеститьОРасшифровкеФайла(ЭтаФорма.Объект.Ссылка);
...
Процедура ПроверитьЭЦП(Команда)
	...
	Иначе
		...
		ДвоичныеДанные = ПолучитьИзВременногоХранилища(
			ПрисоединенныеФайлы.ПолучитьДанныеФайла(ЭтаФорма.Объект.Ссылка).СсылкаНаДвоичныеДанныеФайла);
...
Процедура ЗакончитьРедактирование(Команда)
	...
	Если ПрисоединенныеФайлыКлиент.ПоместитьРедактируемыйФайлНаДискеВХранилище(
			ДанныеФайла,
			ИнформацияОФайле,
			УникальныйИдентификатор) Тогда
...
Процедура ЗанестиИнформациюОПодписях(МассивПодписей)
	...
	ПрисоединенныеФайлы.ЗанестиИнформациюОПодписях(
		ЗаписываемыйОбъект, МассивПодписей, УникальныйИдентификатор);
...
Функция ОбработатьКомандуЗаписиФайла()
	...
	Попытка
		ФайловыеФункцииКлиент.КорректноеИмяФайла(ЭтаФорма.Объект.Наименование);
	Исключение
		Информация = ИнформацияОбОшибке();
		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			Информация.Описание, ,"Наименование", "Объект");
...
Функция ЗаписатьФайл(Знач ПараметрОбъект = Неопределено)
	...
	Попытка
		Если ЗначениеЗаполнено(ЗначениеКопирования) Тогда
			...
			Если ФайловыеФункции.ПолучитьТипХраненияФайлов()
...
Процедура НастроитьОбъектФормы(Знач НовыйОбъект)
	...
	НовыйРеквит = Новый РеквизитФормы("Объект", Новый ОписаниеТипов(ТипНовогоОбъекта));
