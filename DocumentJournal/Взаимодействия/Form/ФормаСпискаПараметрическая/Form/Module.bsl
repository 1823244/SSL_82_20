...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Отбор") Тогда
		Если Параметры.Отбор.Свойство("Предмет") Тогда
			...
			ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(Список.Отбор, "Предмет",
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Элементы.СтраницыСписокДерево.ТекущаяСтраница = 
			Элементы.СтраницаДерево И ВзаимодействияКлиентСервер.ЯвляетсяВзаимодействием(Источник) Тогда
	...
	Если Копирование Тогда
		...
		Если ТекущиеДанные <> Неопределено Тогда
			...
			ИначеЕсли ТипЗнч(ТекущиеДанные.Ссылка) = Тип("ДокументСсылка.Встреча") Тогда
				...
				ОткрытьФормуМодально("Документ.Встреча.ФормаОбъекта",
			...
			ИначеЕсли ТипЗнч(ТекущиеДанные.Ссылка) = Тип("ДокументСсылка.ЗапланированноеВзаимодействие") Тогда
				...
				ОткрытьФормуМодально("Документ.ЗапланированноеВзаимодействие.ФормаОбъекта",
					Новый Структура("ЗначениеКопирования", ТекущиеДанные.Ссылка), ЭтаФорма);
				
			ИначеЕсли ТипЗнч(ТекущиеДанные.Ссылка) = Тип("ДокументСсылка.ТелефонныйЗвонок") Тогда
				
				ОткрытьФормуМодально("Документ.ТелефонныйЗвонок.ФормаОбъекта", 
					Новый Структура("ЗначениеКопирования",ТекущиеДанные.Ссылка), ЭтаФорма);
...
Процедура ОтветственныйВыполнить()
	...
	ВыбранныйОтветственный = 
		ОткрытьФормуМодально("Справочник.Пользователи.Форма.ФормаСписка", Новый Структура("РежимВыбора",Истина));
	
	Если ВыбранныйОтветственный <> Неопределено Тогда
		
		ВыполненаЗамена = Ложь;
		УстановитьОтветственного(ВыбранныйОтветственный,ВыполненаЗамена);
...
Процедура ПредметВыполнить()
	...
	ВыбранныйПредмет = 
		ОткрытьФормуМодально(ВыбранныйТипПредмета.Значение + ".ФормаВыбора");
	
	Если ВыбранныйПредмет <> Неопределено Тогда
		
		Если ОтборПоПредмету И ПредметДляОтбора = ВыбранныйПредмет Тогда
			Возврат;
		КонецЕсли;
		
		ВыполненаЗамена = Ложь;
		УстановитьПредмет(ВыбранныйПредмет,ВыполненаЗамена);
...
Процедура ЗаполнитьДеревоВзаимодействий()
	...
	ИначеЕсли ТипВзаимодействия = "ИсходящиеПисьма" Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ОтборНастройщикаКомпоновок, 
			"СтатусИсходящегоПисьма", ВидСравненияКомпоновкиДанных.Равно, 
			ПредопределенноеЗначение("Перечисление.СтатусыИсходящегоЭлектронногоПисьма.Исходящее"));
