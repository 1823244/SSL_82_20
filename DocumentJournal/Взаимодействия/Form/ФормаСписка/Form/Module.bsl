...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(Закладки.Отбор, "Владелец", Пользователи.ТекущийПользователь(),,, Истина);
	...
	Для Каждого ТипПредмета Из Метаданные.Документы.ЭлектронноеПисьмоИсходящее.Реквизиты.Предмет.Тип.Типы() Цикл
		Если ТолькоПочта И 
			...
			ТипПредмета = Тип("ДокументСсылка.ЗапланированноеВзаимодействие")) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ПапкиПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	ОткрытьФормуМодально("Справочник.ПапкиЭлектронныхПисем.ФормаОбъекта",Новый Структура("ЗначенияЗаполнения",СтруктураПараметры), ЭтаФорма);
...
Процедура СписокПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	ОткрытьФормуМодально("Документ.ЭлектронноеПисьмоИсходящее.ФормаОбъекта", Новый Структура("Вложения",СписокИменФайлов), ЭтаФорма);
...
Процедура ОтветственныйВыполнить(Команда)
	...
	ВыбранныйОтветственный = ОткрытьФормуМодально("Справочник.Пользователи.Форма.ФормаСписка",
		Новый Структура("РежимВыбора",Истина));
	Если ВыбранныйОтветственный <> Неопределено Тогда
		
		УстановитьОтветственного(ВыбранныйОтветственный,Неопределено);
...
Процедура ОтветственныйСписок(Команда)
	...
	Если ВыбранныйОтветственный <> Неопределено Тогда
		...
		УстановитьОтветственного(ВыбранныйОтветственный,Элементы[ИмяТекущегоЭлемента].ВыделенныеСтроки);
...
Процедура ПредметВыполнить(Команда)
	...
	Предмет = ОткрытьФормуМодально(ВыбранныйТипПредмета.Значение + ".ФормаВыбора");
	Если Предмет <> Неопределено Тогда
		УстановитьПредмет(Предмет,Неопределено);
	КонецЕсли;
...
Процедура ПредметСписок(Команда)
	...
	Предмет = ОткрытьФормуМодально(ВыбранныйТипПредмета.Значение + ".ФормаВыбора");
	Если Предмет <> Неопределено Тогда
		УстановитьПредмет(Предмет,Элементы[ИмяТекущегоЭлемента].ВыделенныеСтроки);
	КонецЕсли;
...
Процедура ПеренестиВПапку(Команда)
	...
	ВыбраннаяПапка = ОткрытьФормуМодально("Справочник.ПапкиЭлектронныхПисем.ФормаВыбора", 
	Новый Структура("Отбор",Новый Структура("Владелец", ПапкиТекущиеДанные.УчетнаяЗапись)));
	Если ВыбраннаяПапка <> Неопределено Тогда
		
		Если Лев(ИмяТекущегоЭлемента,6) = "Список" Тогда
			ВыполнитьПереносВПапкуМассиваПисем(Элементы[ИмяТекущегоЭлемента].ВыделенныеСтроки, ВыбраннаяПапка);
		Иначе
			УстановитьРодителяУПапки(ПапкиТекущиеДанные.Значение, ВыбраннаяПапка);
		КонецЕсли;
...
Процедура ПриИзмененииТипаСервер(ОбновлятьПанельНавигации = Истина)
	...
	ВзаимодействияКлиентСервер.ПриИзмененииОтбораТипВзаимодействий(Список.Отбор, ТипВзаимодействия);
	Если ОбновлятьПанельНавигации Тогда
		ОбновитьПанельНавигации();
	КонецЕсли;
...
Функция СоздатьГруппуОтбораПанелиНавигации()
	...
	Возврат ОбщегоНазначенияКлиентСервер.СоздатьГруппуЭлементовОтбора(Список.Отбор.Элементы,"Отбор панели навигации",
...
&НаСервере
Процедура УправлениеВидиомстьюСписков(ТекущаяСтраницаСписок)
...
Процедура ИзменитьОтборСписок(ИмяТаблицы, ДанныеДляОбработки , ИмяСтраницыСписка);
	...
	Если ДанныеДляОбработки.Значение = Нстр("ru = 'Все'") Тогда
		...
		Список.Отбор.Элементы.Удалить(ГруппаОтбора);
		Возврат;
	...
	Если ИмяТаблицы <> "Папки" И Элементы.СтраницыСписок.ТекущаяСтраница <> Элементы.СтраницаСписокОбычная Тогда
		...
		УправлениеВидиомстьюСписков(Элементы.СтраницаСписокОбычная);
	КонецЕсли;
	...
	ИначеЕсли ИмяТаблицы = "Папки" Тогда
			...
			Если ТипЗнч(ДанныеДляОбработки.Значение) = Тип("СправочникСсылка.ПапкиЭлектронныхПисем") Тогда
				...
				Если Элементы.СтраницыСписок.ТекущаяСтраница <> СтраницаСписок Тогда
					...
					УправлениеВидиомстьюСписков(СтраницаСписок);
				КонецЕсли;
...
Процедура УстановитьЗаголовокВидаПанелиНавигации()
	...
	Для каждого ПодчиченныйЭлемент Из Элементы.ВыборВариантаНавигации.ПодчиненныеЭлементы Цикл
		Если ТипЗнч(ПодчиченныйЭлемент) = Тип("КнопкаФормы") Тогда
			ПодчиченныйЭлемент.Пометка = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьДеревоРеквизитовПредмета(ИмяКоманды = "")
	...
	|	РеквизитыПредмета.НеРассмотрено,
	|	РеквизитыПредмета.РеквизитДопУпорядочивания
	|ПОМЕСТИТЬ РеквзитыПредметаСоединения
	|ИЗ
	|РеквизитыПредмета КАК РеквизитыПредмета
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ЖурналДокументов.Взаимодействия КАК Взаимодействия
	...
	|	КОНЕЦ КАК НомерКартинки
	|ИЗ
	|	РеквзитыПредметаСоединения КАК РеквизитыПредмета
	|		ЛЕВОЕ СОЕДИНЕНИЕ ЖурналДокументов.Взаимодействия КАК Взаимодействия
	|		ПО РеквизитыПредмета.Значение = Взаимодействия.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
...
Функция ПолучитьТекстЗапросаПоОтборуСписка(Запрос)
	...
	Если Список.Отбор.Элементы.Количество() > 0 Тогда
		...
		СкопироватьОтбор(КомпоновщикНастроек.Настройки.Отбор,Список.Отбор,,, Истина);
...
Функция ДобавитьВЗакладкиСервер(Знач ДанныеДляОбработки, ИмяЭлементаФормы)
	...
	Иначе
		...
		ГруппаОтбораПоПанелиНавигации = ОбщегоНазначенияКлиентСервер.НайтиЭлементОтбораПоПредставлению(Список.Отбор.Элементы,
...
Функция ПолучитьВзаимодействияОтборуСписка(ЗначениеРеквизитаДопОтбора = Неопределено, ИмяРеквизитаДопОтбора = "")
	...
	СкопироватьОтбор(КомпоновщикНастроек.Настройки.Отбор, Список.Отбор);
	
	//Добавим отбор с видом сравнения НЕ для групповых комманд
	Если ЗначениеРеквизитаДопОтбора <> Неопределено Тогда
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПолнотекстовыйПоиск") И 
		...
		Если ИнформационнаяБазаФайловая Тогда
			...
			Если НЕ ИндексППДАктуален Тогда
				//Текущая дата используется так как дата актуальности полнотекстового поиска это серверное время
...
// Обновить индекс полнотектового поиска.
...
Процедура ПроверитьИндексППД()
	...
	Если Не ИндексППДАктуален Тогда
		...
		Результат = Вопрос(НСтр("ru='Индекс полнотекстового поиска неактуален. Обновить индекс?'"),
