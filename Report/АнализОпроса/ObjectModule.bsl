...
Процедура СформироватьОтчет(ТаблицаОтчета,Опрос,ВидОтчета) Экспорт
	...
	Если ВидОтчета = НСтр("ru='Анализ ответов'") Тогда
		
		СформироватьОтчетАнализОтветов(ТаблицаОтчета,Опрос);
		
	ИначеЕсли ВидОтчета = НСтр("ru='Анализ ответивших'") Тогда
...
Процедура СформироватьОтчетАнализОтветов(ТаблицаОтчета,Опрос)
	...
	РеквизитыОпрос = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Опрос,"ШаблонАнкеты,ДатаНачала,ДатаОкончания,Представление");
...
Функция ВыполнитьЗапросПоВопросамШаблонаАнкеты(Опрос,ШаблонАнкеты)
	...
	|			И (ВопросыДляАнкетирования.ТипОтвета = ЗНАЧЕНИЕ(Перечисление.ТипыОтветовНаВопрос.ОдинВариантИЗ)
	|					ИЛИ ВопросыДляАнкетирования.ТипОтвета = ЗНАЧЕНИЕ(Перечисление.ТипыОтветовНаВопрос.НесколькоВариантовИз))) КАК Вопросы
	|			ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|				ОтветыНаВопросыАнкет.Вопрос КАК Вопрос,
	|				ОтветыНаВопросыАнкет.ЭлементарныйВопрос КАК ЭлементарныйВопрос,
	|				ОтветыНаВопросыАнкет.НомерЯчейки КАК НомерЯчейки,
	...
	|				ОтветыНаВопросыАнкет.Анкета.Опрос = &Опрос
	|				И (ТИПЗНАЧЕНИЯ(ОтветыНаВопросыАнкет.Ответ) = ТИП(БУЛЕВО)
	|						ИЛИ ТИПЗНАЧЕНИЯ(ОтветыНаВопросыАнкет.Ответ) = ТИП(Справочник.ВариантыОтветовАнкет))
	|			
	|			СГРУППИРОВАТЬ ПО
	|				ОтветыНаВопросыАнкет.Вопрос,
	|				ОтветыНаВопросыАнкет.ЭлементарныйВопрос,
	...
	|				И (ВопросыДляАнкетирования.ТипОтвета = ЗНАЧЕНИЕ(Перечисление.ТипыОтветовНаВопрос.Булево)
	|						ИЛИ ВопросыДляАнкетирования.ТипОтвета = ЗНАЧЕНИЕ(Перечисление.ТипыОтветовНаВопрос.ОдинВариантИз)
	|						ИЛИ ВопросыДляАнкетирования.ТипОтвета = ЗНАЧЕНИЕ(Перечисление.ТипыОтветовНаВопрос.НесколькоВариантовИз))) КАК ДанныеРегистра
	...
	|		И (ВопросыДляАнкетирования.ТипОтвета = ЗНАЧЕНИЕ(Перечисление.ТипыОтветовНаВопрос.Булево)
	|				ИЛИ ВопросыДляАнкетирования.ТипОтвета = ЗНАЧЕНИЕ(Перечисление.ТипыОтветовНаВопрос.ОдинВариантИз)
	|				ИЛИ ВопросыДляАнкетирования.ТипОтвета = ЗНАЧЕНИЕ(Перечисление.ТипыОтветовНаВопрос.НесколькоВариантовИз))
	|	
	|	СГРУППИРОВАТЬ ПО
	|		ВопросыШаблонаАнкетыСоставТабличногоВопроса.Ссылка,
	|		ВопросыШаблонаАнкеты.ЭтоГруппа,
...
Функция АгрегатныеЗначенияВСтроку(СтрокаДерева,АгрегироватьСуммуВОтчетах = Неопределено)
	...
	Если ТипЗнч(СтрокаДерева.Максимум) = Тип("Дата") Тогда
		Максимум = Формат(СтрокаДерева.Максимум, "ДФ=dd.MM.yyyy");
	Иначе
		Максимум = Формат(ОКР(СтрокаДерева.Максимум,2),"ЧРД=.");
	КонецЕсли;
	
	Если ТипЗнч(СтрокаДерева.Минимум) = Тип("Дата") Тогда
		Минимум = Формат(СтрокаДерева.Минимум, "ДФ=dd.MM.yyyy");
	Иначе
	...
	+ ?(ПустаяСтрока(Минимум),"",Минимум + СтрокаСлеш)+ ?(ПустаяСтрока(Максимум),"",Максимум + СтрокаСлеш) + ?(ПустаяСтрока(Среднее),"",Среднее)+ 
	...
	АнкетированиеКлиентСервер.УдалитьПоследниеСимволыИзСтроки(СтрокаВозврата,СтрокаСлеш);
...
Процедура СформироватьОтчетАнализОтветивших(ТаблицаОтчета,Опрос)
	...
	РеквизитыОпрос = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Опрос,"ШаблонАнкеты,ТипРеспондентов,СвободныйОпрос,ДатаНачала,ДатаОкончания,Представление");
...
Функция ВыполнитьЗапросПоОтветившимСвободныйОпрос(Опрос,ТипРеспондентов)
	...
	|ИЗ
	|	" + ТипРеспондентов.Метаданные().ПолноеИмя() + " КАК ТаблицаСправочника
	|ГДЕ
	|	(НЕ ТаблицаСправочника.ПометкаУдаления)
	|	" + ?(ТипРеспондентов.Метаданные().Иерархический," И (НЕ ТаблицаСправочника.ЭтоГруппа)","") + "
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
// Формирует представление опроса для заголовка отчёта
