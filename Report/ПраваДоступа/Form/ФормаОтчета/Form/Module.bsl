...
Процедура ВывестиОтчет(Ссылка)
	...
	Если НЕ ВыводПравГруппы Тогда
		...
		ПользовательИБПрочитан = Пользователи.ПрочитатьПользователяИБ(ОбщегоНазначения.ПолучитьЗначениеРеквизита(Ссылка, "ИдентификаторПользователяИБ"), СвойстваПользователяИБ, РолиПользователяИБ);
	...
	Запрос.УстановитьПараметр("СинонимыРолей",       ПользователиСерверПовтИсп.СинонимыРолей());
	Запрос.УстановитьПараметр("СинонимыТаблиц",      УправлениеДоступомСерверПовтИсп.СинонимыТаблиц());
	Запрос.УстановитьПараметр("ПраваОМДРолей",       УправлениеДоступомСерверПовтИсп.ОграничиваемыеПраваОбъектовМетаданныхРолей(Истина));
	...
	Если ПодробныеСведенияОПравахДоступа Тогда
		// Вывод групп доступа
		ОписанияГруппДоступа = РезультатыЗапроса[4].Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкам).Строки;
	...
	Для каждого ОписаниеТипаЗначений Из ПраваПоЗначениям.Строки Цикл
		ПраваПоЗначению = УправлениеДоступом.ВозможныеПраваПоЗначениюДоступа(ОписаниеТипаЗначений.ТипЗначенияПолноеИмя);
		...
		Для каждого ОписаниеОбъекта Из ОписаниеТипаЗначений.Строки Цикл
			...
			Документ.Вывести(Область, 2);
			МакетСтроки.Область("R1C1").Текст = ?(ОписаниеОбъекта.РаспространяетсяВИерархии, "+", "");
			МакетСтроки.Область("R1C1").ШиринаКолонки = 6;
			МакетСтроки.Область("R1C2").Текст = ?(ОписаниеОбъекта.Запрещено, НСтр("ru = 'Запрещено'"), НСтр("ru = 'Разрешено'"));
...
Функция ВидыОграниченийПравОбъектовМетаданных()
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ПостоянныеВидыОграничений", УправлениеДоступомСерверПовтИсп.ПостоянныеВидыОграниченийПравОбъектовМетаданных());
	Запрос.Текст =
	"ВЫБРАТЬ
