...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьЗаголовокКнопкиФормата(Знач КнопкаФормы, Знач ФорматНеУстановлен)
...
Процедура ОткрытьСписокНаборов()
	...
	Если ОткрываетсяИзНабораСвойств Тогда
		Предупреждение(НСтр("ru = 'Свойство открыто из формы набора.
		                          |Редактирование списка наборов не производится.'"));
		Возврат;
	КонецЕсли;
	...
	Результат = ОткрытьФормуМодально(
		"ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения.Форма.ФормаРедактированияСпискаНаборов", ПараметрыФормы);
