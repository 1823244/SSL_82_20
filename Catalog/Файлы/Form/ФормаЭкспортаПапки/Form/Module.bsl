...
Процедура ПапкаДляЭкспортаПриИзменении(Элемент)
	...
	ПапкаДляЭкспортаПолная = ФайловыеФункцииКлиент.НормализоватьКаталог(ПапкаДляЭкспортаПолная);
КонецПроцедуры

&НаКлиенте
Процедура ПапкаДляЭкспортаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	#Если НЕ ВебКлиент Тогда
		// Откроем окно выбора папки сохранения
		СтандартнаяОбработка = Ложь;
		ВыборФайла = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.ВыборКаталога);
		ВыборФайла.МножественныйВыбор = Ложь;
		ВыборФайла.Каталог = ПапкаДляЭкспорта;
		Если ВыборФайла.Выбрать() Тогда
			ПапкаДляЭкспорта = ФайловыеФункцииКлиент.НормализоватьКаталог(ВыборФайла.Каталог);
			ПапкаДляЭкспортаПолная = ПапкаДляЭкспорта + Строка(ЧтоСохраняем) + ФайловыеФункцииКлиентСервер.ПолучитьСлеш(ОбщегоНазначенияКлиентПовтИсп.ТипПлатформыКлиента());
		КонецЕсли;
	#КонецЕсли
КонецПроцедуры
		...
		Для Каждого ЗаписьФайла из ТаблицаФайлов.ПолучитьЭлементы() Цикл
			...
			Если (ЗаписьФайла.Папка <> ЧтоСохраняем) И ЗаписьФайла.ТекущаяВерсия.Пустая() Тогда
				Если РодительскаяПапка <> ЗаписьФайла.Папка Тогда
					БазовыйКаталогСохраненияФайла = БазовыйКаталогСохраненияФайла + ЗаписьФайла.НаименованиеПапки + ФайловыеФункцииКлиентСервер.ПолучитьСлеш(ОбщегоНазначенияКлиентПовтИсп.ТипПлатформыКлиента());
				КонецЕсли;
			КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	// Установим в качестве папки выгрузки Мои Документы
	// или ту папку, в которую выгружали файлы последний раз
	ПапкаДляЭкспорта = ФайловыеФункцииКлиент.КаталогВыгрузки();
	ПапкаДляЭкспортаПолная = ПапкаДляЭкспорта + Строка(ЧтоСохраняем) + ФайловыеФункцииКлиентСервер.ПолучитьСлеш(ОбщегоНазначенияКлиентПовтИсп.ТипПлатформыКлиента());
