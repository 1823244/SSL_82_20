...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Чтение
	Если НЕ ЭтоНовый ИЛИ СозданКопированием Тогда
		ПрочитатьПользовательскоеПредставлениеФорматов();
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ПроверитьПерсонализированныеОтборы = (НЕ Объект.Персонализирована И РассылкаБылаПерсонализирована);
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		...
		Если СтрокаОтчеты.ВнесеныИзменения Тогда
			...
			Если ТипЗнч(НастройкиОтчета) = Тип("КомпоновщикНастроекКомпоновкиДанных") Тогда //СтрокаОтчеты.СКД Тогда
				
				СохраняемоеЗначение = НастройкиОтчета.ПользовательскиеНастройки;
				
			ИначеЕсли ТипЗнч(НастройкиОтчета) = Тип("ТаблицаЗначений") Тогда
				
				ОтборТаблицы = Новый Структура("Обнаружен", Истина);
				КолонкиТаблицы = "Реквизит, Представление, Значение, Использование";
				СохраняемоеЗначение = НастройкиОтчета.Скопировать(ОтборТаблицы, КолонкиТаблицы);
			...
			КонецЕсли;
			ОбъектСтрокаОтчеты.Настройки = Новый ХранилищеЗначения(СохраняемоеЗначение, Новый СжатиеДанных(9));
		КонецЕсли;
		
		// Поиск и отключение персонализированных полей
		Если ПроверитьПерсонализированныеОтборы Тогда
			НастройкиОтчета = ОбъектСтрокаОтчеты.Настройки.Получить();
			ОтчетПерсонализирован = Ложь;
			Если ТипЗнч(НастройкиОтчета) = Тип("ПользовательскиеНастройкиКомпоновкиДанных") Тогда
				Для Каждого СтрокаНастроек Из НастройкиОтчета.Элементы Цикл
					Если ТипЗнч(СтрокаНастроек) = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
						ЗначениеНастройки = СтрокаНастроек.ПравоеЗначение;
					ИначеЕсли ТипЗнч(СтрокаНастроек) = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
						ЗначениеНастройки = СтрокаНастроек.Значение;
					Иначе
						Продолжить;
					КонецЕсли;
					Если ЗначениеНастройки = "[Получатель]" И СтрокаНастроек.Использование Тогда
						ОтчетПерсонализирован = Истина;
						Прервать; 
					КонецЕсли;
				КонецЦикла; // Для Каждого СтрокаНастроек Из Настройки.Элементы Цикл
			ИначеЕсли ТипЗнч(НастройкиОтчета) = Тип("ТаблицаЗначений") Тогда
			...
			Если ОтчетПерсонализирован Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В параметрах отчета %1 задан отбор по получателям рассылки, необходимо отключить отбор по получателям или изменить вид рассылки на ''Свой отчет для каждого получателя''.'"),
			...
			КонецЕсли;
		КонецЕсли; // ПроверитьПерсонализированныеОтборы
...
Процедура ПапкаОткрытие(Элемент, СтандартнаяОбработка)
	РассылкаОтчетовКлиентПереопределяемый.ОткрытьФормуПапки(СтандартнаяОбработка, Объект.Папка);
КонецПроцедуры
...
Процедура ДобавитьОтчет(Команда)
	Пользовательский = ПредопределенноеЗначение("Перечисление.ТипыВариантовОтчетов.Пользовательский");
	...
	ОтборПоТипу = Новый СписокЗначений;
	ОтборПоТипу.Добавить(Пользовательский, Пользовательский);
	ОтборПоТипу.Добавить(Предопределенный, Предопределенный);
	ОтборПоТипу.Добавить(Отчет, Отчет);
	...
	ПараметрыФормыВыбора.Вставить("ОтборПоТипу", ОтборПоТипу);
...
Процедура ДобавитьДополнительныйОтчет(Команда)
	РассылкаОтчетовКлиентПереопределяемый.ПодборДопОтчетов(Элементы.Отчеты);
КонецПроцедуры
...
Процедура ПрочитатьНастройкиОтчетовОбъектаКопирования()
	...
	ПричинаУдаленияСтрок = "";
	
	Для Индекс = 1 По Объект.Отчеты.Количество() Цикл
		СтрокаОтчеты = Объект.Отчеты[Индекс - 1];
		СтрокаОтчетыОбъектаКопирования = ЗначениеКопирования.Отчеты[Индекс - 1];
		
		НастройкиОтчета = СтрокаОтчетыОбъектаКопирования.Настройки.Получить();
		
		ПараметрыОтчета = ИнициалиазироватьОтчет(СтрокаОтчеты.Отчет, НастройкиОтчета);
		
		// Проверка результата инциализации
		Если НЕ ПараметрыОтчета.Инициализирован Тогда
			МассивУдаляемыхСтрок.Добавить(СтрокаОтчеты);
			ПричинаУдаленияСтрок = ПричинаУдаленияСтрок + ?(ПричинаУдаленияСтрок = "", "", Символы.ПС) 
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Отчет %1 был удален из-за ошибки инициализации (см. выше).'"),
		...
		Иначе
			...
			Если ТипЗнч(НастройкиОтчета) = Тип("ТаблицаЗначений") Тогда
				НастройкиТекущегоОтчета.Загрузить(НастройкиОтчета);
			Иначе
			...
			Для Каждого КлючИЗначение Из ПараметрыОтчета.ДоступныеРеквизиты Цикл
				...
				СтрокаНастройка.ИндексКартинки = 2;
			КонецЦикла;
		...
		СтрокаОтчеты.АдресНастроек = ПоместитьВоВременноеХранилище(
			?(СтрокаОтчеты.СКД, 
				КомпоновщикНастроекКД,
				НастройкиТекущегоОтчета.Выгрузить()
			), 
			?(ЭтоАдресВременногоХранилища(СтрокаОтчеты.АдресНастроек),
				СтрокаОтчеты.АдресНастроек,
	...
	Если МассивУдаляемыхСтрок.Количество() > 0 Тогда
		Для Каждого СтрокаОтчеты Из МассивУдаляемыхСтрок Цикл
			Объект.Отчеты.Удалить(СтрокаОтчеты);
		КонецЦикла;
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = ПричинаУдаленияСтрок;
		Сообщение.Сообщить();
	КонецЕсли;
КонецПроцедуры
...
&НаСервере
Функция ОтчетыПриАктивизацииСтрокиНаСервере(ИдентификаторСтроки, ДобавитьДежурнуюФразу = Истина)
	...
	Если СтрокаОтчеты = Неопределено Тогда
		ИдентификаторТекущейСтрокиТаблицыОтчетов = -1;
		Возврат НСтр("ru = 'Не выбрана строка отчета'");
	КонецЕсли;
	
	// Чтение настроек текущей строки из временного хранилища + переключение страницы
	Если ЭтоАдресВременногоХранилища(СтрокаОтчеты.АдресНастроек) Тогда
		...
		Если ТипЗнч(Настройки) = Тип("КомпоновщикНастроекКомпоновкиДанных") Тогда //СтрокаОтчеты.СКД Тогда
			КомпоновщикНастроекКД = Настройки;
		...
		ИначеЕсли ТипЗнч(Настройки) = Тип("ТаблицаЗначений") Тогда
			НастройкиТекущегоОтчета.Загрузить(Настройки);
			Элементы.СтраницыНастроекОтчета.ТекущаяСтраница = Элементы.СтраницаНастройкиТекущегоОтчета;
		Иначе
			Элементы.СтраницыНастроекОтчета.ТекущаяСтраница = Элементы.ПустаяСтраница;
		КонецЕсли;
	...
	// Чтение настроек отчета по ссылке
	СтрокаОтчетыПоСсылке = Объект.Ссылка.Отчеты.Найти(СтрокаОтчеты.Отчет, "Отчет");
	НастройкиОтчета = ?(СтрокаОтчетыПоСсылке = Неопределено, Неопределено, СтрокаОтчетыПоСсылке.Настройки.Получить());
	
	// Инициализация
	ПараметрыОтчета = ИнициалиазироватьОтчет(СтрокаОтчеты.Отчет, НастройкиОтчета);
	
	// Очистка сообщений окна
	Ошибки = РассылкаОтчетовКлиентСервер.СтрокаСообщенийПользователю(ПараметрыОтчета.МассивОшибок, ДобавитьДежурнуюФразу);
	ПараметрыОтчета.Ошибки = ПараметрыОтчета.Ошибки 
		+ ?(ПараметрыОтчета.Ошибки = "" ИЛИ Ошибки = "", "", Символы.ПС + Символы.ПС)
		+ Ошибки;
	...
	Если НЕ СтрокаОтчеты.Инициализирован Тогда
		// Удаление строки, если она не была записана в БД
		Если СтрокаОтчетыПоСсылке = Неопределено Тогда
			Объект.Отчеты.Удалить(СтрокаОтчеты);
		КонецЕсли;
...
&НаСервере
Функция ИнициалиазироватьОтчет(Отчет, НастройкиОтчета)
	// Параметры журнала
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия",   НСтр("ru = 'Рассылка отчетов. Инициализация отчета'"));
	...
	ПараметрыОтчета = Новый Структура("Отчет, Настройки", Отчет, НастройкиОтчета);
...
Функция СформироватьОтчетНаСервере(Знач ПараметрыОтчета, Получатель, ТребуемыеРеквизиты)
	ПараметрыЖурнала = Новый Структура;
	ПараметрыЖурнала.Вставить("ИмяСобытия", НСтр("ru = 'Рассылка отчетов. Формирование отчета'"));
...
Функция ПравоИзмененияПапкиИФайлов(Папка)
	...
	Результат = Истина;
	РассылкаОтчетовПереопределяемый.ПравоИзмененияПапкиИФайлов(Папка, Результат);
	Возврат Результат;
...
Процедура ПереключитьСтраницу(Элементы, ИмяНабораСтраниц, ПрефиксСтраниц = "", СуффиксСтраниц)
	ИмяТекущейСтраницы = ?(ПрефиксСтраниц = "", ИмяНабораСтраниц, ПрефиксСтраниц) + СуффиксСтраниц;
	Элементы[ИмяНабораСтраниц].ТекущаяСтраница = Элементы[ИмяТекущейСтраницы];
КонецПроцедуры
