		...
		Если НЕ ЭтоНовый() Тогда
			...
			Если Наименование <> Ссылка.Наименование Тогда // переименована папка
				...
				Если РабочийКаталог <> "" Тогда
					...
					РабочийКаталогУнаследованныйПрежний = РабочийКаталог + Ссылка.Наименование + ФайловыеФункцииКлиентСервер.ПолучитьСлеш(ОбщегоНазначенияПовтИсп.ТипПлатформыСервера());
					Если РабочийКаталогУнаследованныйПрежний = РабочийКаталогЭтойПапки Тогда
						РабочийКаталог = РабочийКаталог + Наименование + ФайловыеФункцииКлиентСервер.ПолучитьСлеш(ОбщегоНазначенияПовтИсп.ТипПлатформыСервера());
			...
			Если Родитель <> Ссылка.Родитель Тогда // перенесли папку в другую папку
				...
				Если РабочийКаталог <> "" ИЛИ РабочийКаталогНовый <> "" Тогда
					...
					Если РабочийКаталог <> "" Тогда
						РабочийКаталогУнаследованныйПрежний = РабочийКаталогУнаследованныйПрежний + Ссылка.Наименование + ФайловыеФункцииКлиентСервер.ПолучитьСлеш(ОбщегоНазначенияПовтИсп.ТипПлатформыСервера());
					КонецЕсли;
					...
					Если РабочийКаталогУнаследованныйПрежний = РабочийКаталогЭтойПапки Тогда // рабочий каталог автоформируется от родителя
						...
						Если РабочийКаталогНовый <> "" Тогда
							РабочийКаталогНовый = РабочийКаталогНовый + Наименование + ФайловыеФункцииКлиентСервер.ПолучитьСлеш(ОбщегоНазначенияПовтИсп.ТипПлатформыСервера());
	...
	Если НЕ ОбменДанными.Загрузка Тогда
		...
		Если ДополнительныеСвойства.ПрошлыйЭтоНовый Тогда
			...
			Если РабочийКаталогВладельца <> "" Тогда
				...
				РабочийКаталогВладельца = РабочийКаталогВладельца + Наименование + ФайловыеФункцииКлиентСервер.ПолучитьСлеш(ОбщегоНазначенияПовтИсп.ТипПлатформыСервера());
