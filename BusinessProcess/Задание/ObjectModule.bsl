...
Процедура ЗаполнитьНаборыЗначенийДоступа(Таблица) Экспорт
	...
	Если ЗначениеЗаполнено(Предмет) Тогда
		...
		ИначеЕсли УправлениеДоступом.ВозможноЗаполнитьНаборыЗначенийДоступа(Предмет) Тогда
...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ЗадачаИсточник.Пустая() Тогда
		...
		Если Завершен = Истина И БылЗавершен = Ложь Тогда
			БизнесПроцессыИЗадачиПереопределяемый.ВыполнитьЗадачуИсточник(ЭтотОбъект);
		КонецЕсли;	
	КонецЕсли;	
	...
	Если БизнесПроцессыИЗадачиПереопределяемый.ИспользоватьВнешниеЗадачиИБизнесПроцессы() Тогда
		Если ТипЗнч(Исполнитель) = Тип("СправочникСсылка.РолиИсполнителей") И Не Исполнитель.Пустая() 
			И ОбщегоНазначения.ПолучитьЗначениеРеквизита(Исполнитель, "ВнешняяРоль") = Истина Тогда
...
Процедура ВыполнитьПередСозданиемЗадач(ТочкаМаршрутаБизнесПроцесса, ФормируемыеЗадачи, СтандартнаяОбработка)
	...
	Если БизнесПроцессыИЗадачиПереопределяемый.ИспользоватьВнешниеЗадачиИБизнесПроцессы() Тогда
		...
		Если ТипЗнч(Исполнитель) = Тип("СправочникСсылка.РолиИсполнителей") 
			...
			СтрокаHTML = БизнесПроцессыИЗадачиПереопределяемый.ПолучитьПредставлениеПредметаВнешнейЗадачи(Предмет);
			СодержаниеПредмета = Новый ХранилищеЗначения(СтрокаHTML, Новый СжатиеДанных());
			
			// Заполняем список файлов
			МассивФайлов = БизнесПроцессыИЗадачиПереопределяемый.ПолучитьСписокФайлов(Предмет);
...
Процедура ЗавершениеПриЗавершении(ТочкаМаршрутаБизнесПроцесса, Отказ)
	...
	ДатаЗавершения = БизнесПроцессыИЗадачиВызовСервера.ДатаЗавершенияБизнесПроцесса(Ссылка);
	Записать();
