...
Функция ПолучитьДанныеИнформационнойБазы(ПолноеИмяТаблицы)
	...
	Результат.СвойстваОбъектаМетаданных = ОбменДаннымиСервер.СвойстваОбъектаМетаданных(ПолноеИмяТаблицы);
	Результат.ТаблицаБазыКорреспондента = ОбменДаннымиСервер.ПолучитьОбъектыТаблицы(ПолноеИмяТаблицы);
...
Функция SaveFileFromParts(TransferId, PartQuantity, FileId)
	...
	Для НомерЧасти = 1 По PartQuantity Цикл
		...
		Если НайтиФайлы(ИмяФайла).Количество() = 0 Тогда
			ШаблонСообщения = НСтр("ru = 'Не найден фрагмент %1 сессии передачи с идентификатором %2.
					|Необходимо убедиться, что в форме настройки программы заданы значения констант
					|КаталогВременныхФайловСообщенийОбменаДляLinux и КаталогВременныхФайловСообщенийОбменаДляWindows.'"
			);
...
Функция ЗарегистрироватьДанныеДляНачальнойВыгрузки(Знач ИмяПланаОбмена, Знач КодУзла, ДлительнаяОперация, ИдентификаторОперации, ТолькоСправочники)
	...
	Данные = ?(ТолькоСправочники,
				ОбменДаннымиСервер.СправочникиПланаОбмена(ИмяПланаОбмена),
				ОбменДаннымиСервер.ВсеДанныеПланаОбменаКромеСправочников(ИмяПланаОбмена)
	);
	...
	Иначе
		...
		ФоновоеЗадание = ФоновыеЗадания.Выполнить("ОбменДаннымиСервер.ЗарегистрироватьДанныеДляНачальнойВыгрузки",
											Параметры,
											,
											НСтр("ru = 'Создание обмена данными.'")
		);
