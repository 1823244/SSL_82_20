...
Функция ПолучитьФайл(КодФайла) Экспорт
	...
	Если ВерсияСсылка.Том.Пустая() Тогда
		ВызватьИсключение "Version not found.";
	КонецЕсли;	
		
	ТипХраненияФайлов = ВерсияСсылка.ТипХраненияФайла;
	...
	Иначе
		...
		Попытка
			ДанныеФайла.BinaryData = Новый ДвоичныеДанные(ПолныйПуть);
		Исключение
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Ошибка получения файла из тома. Файл: ""%1.%2""'"),
				ВерсияСсылка.ПолноеНаименование,
				ВерсияСсылка.Расширение);
				
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Получение файла'"), 
				УровеньЖурналаРегистрации.Ошибка, 
				Метаданные.Справочники.Файлы, 
				ВерсияСсылка.Владелец, 
				СообщениеОбОшибке);
			
			ВызватьИсключение "Invalid file path";	
		КонецПопытки;
	КонецЕсли;	
...
Функция ДобавитьФайл(ПапкаXDTO, ФайлXDTO) Экспорт
	...
	НовыйФайл = РаботаСФайламиВызовСервера.СоздатьФайлСВерсией(
		Папка,
		ФайлXDTO.Name,
		ФайловыеФункцииКлиентСервер.РасширениеБезТочки(ФайлXDTO.Extension),
		ВремяИзменения,
